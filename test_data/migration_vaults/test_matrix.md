...
|---|------|--------|---------|------------|-------|----------|-----------|------------|
| 3.1 | New passphrase vault + 1 login; lock, close, reopen | unlocks | pass | n/a | | | | held |
| 3.2 | New passphrase+2-key vault; reopen with EACH key | unlocks on each | n/a | pass | n/a | | n/a | held |
| 3.3 | Version byte on both | `0b` | pass | pass | | | | held |
...
|---|------|--------|-------|-----|------------|
| 4.1 | Add `v10.gabbro` (Android: `adb push` to `/sdcard/Download/` first) | listed | pass | | held |
| 4.2 | Unlock with correct passphrase `0123456789a` | version-not-supported card; not error-red; no "corrupt"; no Delete; no Restore | pass | | held |
| 4.3 | The card shows a tappable link | link present | **fail** | | held |
| 4.4 | Tap link | opens `docs/VAULT_UPGRADE_PATH.md` | blocked by 4.3 | | held |
| 4.5 | Set app language to Spanish, repeat 4.2 | card is in Spanish | **fail** | | held |
| 4.6 | Close card | app usable; other vaults open | pass | | held |
| 4.7 | Unlock with a WRONG passphrase | same version card, **not** "wrong passphrase" | | | held |

## Step 6 — refusal was non-destructive

| # | Step | Expect | Linux | S23 | GrapheneOS |
|---|------|--------|-------|-----|------------|
| 5.1 | `sha256sum v10.gabbro` | identical to 1.3 | | | held |
| 5.2 | Version byte | still `0a` | | | held |
| 5.3 | Directory beside it | no `.bak`, no new file | | | held |
...
| # | Step | Expect | Linux | S23 | GrapheneOS |
|---|------|--------|-------|-----|------------|
| 6.1 | From a v11 vault, sync/merge `v10.gabbro` | clear refusal, no partial merge | | | held |
| 6.2 | Both files after | unchanged | | | held |

## Step 8 — v11 stays v11 under use

| # | Step | Expect | Linux p | Linux p+yk | S23 p | S23 p+yk | S23 p+bio | GrapheneOS |
|---|------|--------|---------|------------|-------|----------|-----------|------------|
| 7.1 | Add, edit, delete an entry; lock; reopen | survivors intact | | | | | | held |
| 7.2 | Version byte | `0b` | | | | | | held |
| 7.3 | Change passphrase | reopens with NEW only; old rejected | | | | | | held |
| 7.4 | Add a key, remove one (keep >=1), change passphrase | opens with each surviving key + new passphrase; removed key and old passphrase rejected | n/a | | n/a | | n/a | held |
| 7.5 | Version byte after 7.3/7.4 | `0b` | | | | | | held |

## Step 9 — real vault, READ-ONLY

| # | Step | Expect | Linux | S23 | GrapheneOS |
|---|------|--------|-------|-----|------------|
| 8.1 | Open the real vault once per upgraded device | opens, entries visible | | | held |
...
| # | Step | Expect | Linux | S23 | GrapheneOS |
|---|------|--------|-------|-----|------------|
| 9.1 | About -> licences | no `ml-kem`, no `x25519-dalek`; rest intact | | | held |
...
| Ref | Failure |
|-----|---------|
| 4.3 | No tappable link on the version-not-supported card |
| 4.5 | Card text stays English under a non-English UI locale |

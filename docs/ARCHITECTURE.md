...
Next task: **Release v0.1.0-alpha.5** — full `cargo test -q` + `flutter test` gate (both green), then tag + artifacts. Bundles: in-app help carousel, Phases 1–3 (dependency audit, licence audit, header integrity / VERSION 7), multi-language expansion (33 user-facing locales + passphrase generator 20-language expansion).

Pre-release gate: add CC-BY-4.0 attribution for Estonian (`et`) and Bulgarian (`bg`) wordlists to `about_screen.dart` (see § Step 3 wordlist table).
...
**Rust + Flutter: DONE.** `Language` enum (20 variants) lives in `rust/src/api/types.rs` → `lib/src/rust/api/types.dart`. `passphrase_generator.rs` imports it and covers all 20 wordlists. `PasswordConfig` gains `language: Language` with script-aware pools (Greek 24-letter, Cyrillic ru/uk/bg). `generator_widget.dart` uses a scrollable `DropdownButtonFormField<Language>` sorted alphabetically; `_language` drives both passphrase and classic modes.

...
**The dumpable↔XDG-portal trap (a regression this caused).** `PR_SET_DUMPABLE(0)` and
`file_picker` **cannot coexist** in the same process, by kernel design. A non-dumpable process has
its `/proc/<pid>/{root,cwd,exe}` symlinks reassigned to a `ptrace_may_access`-gated state: a
same-uid peer *without* `CAP_SYS_PTRACE` gets `EACCES` dereferencing them (`cgroup`/`mountinfo`
stay world-readable — it's specifically the symlinks). `xdg-desktop-portal` reads exactly those
entries to build the **caller's** app-info when servicing a FileChooser request, so once R-04
landed, *every* Linux file dialog (sync, export, import, restore, attach) failed — surfacing as the
"portal unreachable" SnackBar on a perfectly normal desktop, not just a sandbox. No test caught it
because none invokes the real portal. Resolution: keep `DUMPABLE=0` as the baseline but **raise it
only for the picker window** — `runPicker` (`safe_file_picker.dart`) brackets every dialog with
`setProcessDumpable(true/false)` (ref-counted for nesting). During that brief, user-initiated
window the kernel's yama `ptrace_scope` (≥1 on Debian/Mint and Arch defaults) still blocks any
...
`show_vault_list` (default OFF) was meant as coercion-resistance: the login screen would show only
the last-used vault, so someone with device access couldn't even *see* that other vaults exist. But
the registry it hid — `~/.config/gabbro/vaults.jsonc` — is **plaintext**, listing every vault's alias
and full path. So the toggle only hid vaults from someone who *merely opened the app*, never from the
adversary it targeted, who reads the file directly. We were paying real complexity (a delete-time
leak/orphan bug class, a "can't delete the active vault while siblings exist" rule, special-casing
across five files + four localized strings) for protection the storage layer already defeated. **A
half-strength privacy feature is worse than none — it implies a guarantee you don't provide.** We
removed it: the login screen always lists vaults (every mainstream manager does), and any real
hidden-vault capability would require *encrypting the registry* — a storage change, not a UI flag.
General rule: before adding a privacy control, ask what an attacker who can read the underlying file
already sees; if the toggle doesn't change that, it's theatre.

---

## Testing — navigation tests can't route to FFI-on-mount screens under `flutter test`

A widget test that drove `GabbroApp`'s post-delete routing (call a state method → assert which screen
it lands on) **hung forever**, timing out the whole suite. Cause: the routed-to screens build real
bridge calls *on mount* — `UnlockScreen._probeVault` awaits `onVaultIsReadable` (FFI), the delete

// This file is automatically generated, so please do not edit it.
// @generated by `flutter_rust_bridge`@ 2.13.0.

// ignore_for_file: unused_import, unused_element, unnecessary_import, duplicate_ignore, invalid_use_of_internal_member, annotate_overrides, non_constant_identifier_names, curly_braces_in_flow_control_structures, prefer_const_literals_to_create_immutables, unused_field

import 'api/autotype_bridge.dart';
import 'api/entropy.dart';
import 'api/fido_bridge.dart';
import 'api/import.dart';
import 'api/passkey_bridge.dart';
import 'api/passkey_daemon_bridge.dart';
import 'api/passphrase_generator.dart';
import 'api/password_generator.dart';
import 'api/simple.dart';
import 'api/types.dart';
import 'api/vault.dart';
import 'api/vault_bridge.dart';
import 'dart:async';
import 'dart:convert';
import 'frb_generated.dart';
import 'frb_generated.io.dart'
    if (dart.library.js_interop) 'frb_generated.web.dart';
import 'package:flutter_rust_bridge/flutter_rust_bridge_for_generated.dart';

/// Main entrypoint of the Rust API
class RustLib extends BaseEntrypoint<RustLibApi, RustLibApiImpl, RustLibWire> {
  @internal
  static final instance = RustLib._();

  RustLib._();

  /// Initialize flutter_rust_bridge
  static Future<void> init({
    RustLibApi? api,
    BaseHandler? handler,
    ExternalLibrary? externalLibrary,
    bool forceSameCodegenVersion = true,
  }) async {
    await instance.initImpl(
      api: api,
      handler: handler,
      externalLibrary: externalLibrary,
      forceSameCodegenVersion: forceSameCodegenVersion,
    );
  }

  /// Initialize flutter_rust_bridge in mock mode.
  /// No libraries for FFI are loaded.
  static void initMock({required RustLibApi api}) {
    instance.initMockImpl(api: api);
  }

  /// Dispose flutter_rust_bridge
  ///
  /// The call to this function is optional, since flutter_rust_bridge (and everything else)
  /// is automatically disposed when the app stops.
  static void dispose() => instance.disposeImpl();

  @override
  ApiImplConstructor<RustLibApiImpl, RustLibWire> get apiImplConstructor =>
      RustLibApiImpl.new;

  @override
  WireConstructor<RustLibWire> get wireConstructor =>
      RustLibWire.fromExternalLibrary;

  @override
  Future<void> executeRustInitializers() async {
    await api.crateApiSimpleInitApp();
  }

  @override
  ExternalLibraryLoaderConfig get defaultExternalLibraryLoaderConfig =>
      kDefaultExternalLibraryLoaderConfig;

  @override
  String get codegenVersion => '2.13.0';

  @override
  int get rustContentHash => 441199714;

  static const kDefaultExternalLibraryLoaderConfig =
      ExternalLibraryLoaderConfig(
        stem: 'rust_lib_gabbro',
        ioDirectory: 'rust/target/release/',
        webPrefix: 'pkg/',
        wasmBindgenName: 'wasm_bindgen',
      );
}

abstract class RustLibApi extends BaseApi {
  Future<String> crateApiVaultBridgeAddAttachment({
    required String entryId,
    required String name,
    required String kind,
    required List<int> data,
  });

  Future<void> crateApiVaultBridgeAddYubikey({
    required List<int> newCredId,
    required List<int> newHmacSecret,
    required List<int> newSalt,
  });

  Future<void> crateApiVaultBridgeAdoptVaultFile({
    required String source,
    required String dest,
  });

  Future<void> crateApiVaultBridgeApplySyncDecisions({
    required List<SyncFieldResolutionInput> fieldResolutions,
    required List<SyncHistoryReplacementInput> historyReplacements,
    required List<SyncItemDeleteInput> itemDeletes,
    required List<SyncFolderInput> folders,
    required List<String> entryDeletes,
  });

  Future<String> crateApiPasskeyBridgeAssertionResponseJson({
    required String entryId,
    String? clientDataJsonB64Url,
    String? clientDataHashB64Url,
  });

  Future<void> crateApiVaultBridgeAssignFolderToEntries({
    required List<String> ids,
    required String folder,
  });

  Future<CapturedWindowData?>
  crateApiAutotypeBridgeAutotypeCaptureActiveWindow();

  Future<void> crateApiAutotypeBridgeAutotypeFill({
    required int windowId,
    required String entryId,
  });

  Future<String> crateApiAutotypeBridgeAutotypeSocketPath();

  Future<String> crateApiAutotypeBridgeAutotypeTriggerToken();

  Future<ExportArtifact> crateApiVaultBridgeBuildExportBytes({
    required String vaultFilename,
  });

  Future<ExportArtifact> crateApiVaultBridgeBuildExportPassphraseOnlyBytes({
    required String vaultFilename,
  });

  Future<void> crateApiVaultBridgeCancelSync();

  Future<void> crateApiVaultBridgeChangePassphrase({
    required List<int> oldPassphrase,
    required List<int> newPassphrase,
  });

  Future<String> crateApiVaultChronoNow();

  Future<EntrySummaryData> crateApiVaultBridgeCreateEntry({
    required VaultEntryData entry,
  });

  Future<void> crateApiVaultBridgeCreateFolder({required String name});

  Future<LoginEntryData> crateApiVaultCreateLoginEntry({
    required String folder,
    required String title,
    required String url,
    required String username,
    required String password,
    String? notes,
    required List<CustomFieldData> customFields,
  });

  Future<void> crateApiVaultBridgeDeleteEntries({required List<String> ids});

  Future<void> crateApiVaultBridgeDeleteEntry({required String id});

  Future<void> crateApiVaultBridgeDeleteFolder({
    required String name,
    String? reassignTo,
  });

  Future<void> crateApiVaultBridgeDeleteHistory({
    required String id,
    required int index,
  });

  Future<void> crateApiVaultBridgeDeleteWholeVault();

  double crateApiPasswordGeneratorEntropyBits({
    required int poolSize,
    required int length,
  });

  EntropyResult crateApiEntropyEstimateEntropy({required String password});

  Future<void> crateApiVaultBridgeExportVault({required String path});

  Future<void> crateApiVaultBridgeExportVaultJson({required String path});

  Future<void> crateApiVaultBridgeExportVaultPassphraseOnly({
    required String path,
  });

  Future<Uint8List> crateApiVaultBridgeExtractAttachment({
    required String entryId,
    required String uuid,
  });

  Future<MergeSummary> crateApiVaultBridgeFastMergeVaultFromFile({
    required String path,
    required List<int> passphrase,
  });

  Future<MergeSummary?> crateApiVaultBridgeFastMergeVaultFromFileHeld({
    required String path,
  });

  Future<MergeSummary> crateApiVaultBridgeFastMergeVaultFromFileWithKey({
    required String path,
    required List<int> passphrase,
    required List<int> hmacSecret,
    required List<int> credentialId,
  });

  Future<MergeSummary?> crateApiVaultBridgeFastMergeVaultFromFileWithKeyHeld({
    required String path,
    required List<int> hmacSecret,
    required List<int> credentialId,
  });

  Future<Uint8List> crateApiFidoBridgeFidoGetHmacSecret({
    required String devicePath,
    required List<int> credentialId,
    required List<int> salt,
    required String pin,
  });

  Future<FidoHmacMatch> crateApiFidoBridgeFidoGetHmacSecretAny({
    required String devicePath,
    required List<FidoRecordInput> records,
    required String pin,
  });

  List<String> crateApiFidoBridgeFidoListDevices();

  Future<FidoCredentialData> crateApiFidoBridgeFidoRegister({
    required String devicePath,
    required String pin,
  });

  Future<String> crateApiPassphraseGeneratorGeneratePassphrase({
    required PassphraseConfig config,
  });

  String crateApiPasswordGeneratorGeneratePassword({
    required PasswordConfig config,
  });

  VaultEntryData crateApiVaultBridgeGetEntry({required String id});

  Future<List<HistoryRecordData>> crateApiVaultBridgeGetEntryHistory({
    required String id,
  });

  Future<ImportResult> crateApiImportImportFromBitwarden({
    required List<int> data,
  });

  Future<ImportResult> crateApiImportImportFromCsv({
    required String input,
    required CsvImportConfigData config,
  });

  Future<ImportResult> crateApiImportImportFromDashlane({
    required List<int> data,
  });

  Future<ImportResult> crateApiImportImportFromEnpass({
    required List<int> data,
  });

  Future<GabbroImportResult> crateApiImportImportFromGabbro({
    required String path,
    required List<int> passphrase,
  });

  Future<GabbroImportResult> crateApiImportImportFromGabbroWithKey({
    required String path,
    required List<int> passphrase,
    required List<int> hmacSecret,
    required List<int> credentialId,
  });

  Future<ImportResult> crateApiImportImportFromGooglePm({
    required List<int> data,
  });

  Future<void> crateApiSimpleInitApp();

  Future<void> crateApiVaultBridgeInitVault({
    required List<int> passphrase,
    required String path,
    String? alias,
  });

  Future<void> crateApiVaultBridgeInitVaultWithKeys({
    required List<int> passphrase,
    required List<YubiKeyInitData> keys,
    required String path,
    String? alias,
  });

  List<EntrySummaryData> crateApiVaultBridgeListEntrySummaries();

  List<String> crateApiVaultBridgeListFolders();

  List<YubikeyRecordData> crateApiVaultBridgeListVaultYubikeyRecords({
    required String path,
  });

  List<YubikeyAliasData> crateApiVaultBridgeListYubikeyAliases();

  void crateApiVaultBridgeLockVault();

  Future<MergeSummary> crateApiVaultBridgeMergeVaultFromFile({
    required String path,
    required List<int> passphrase,
  });

  Future<MergeSummary?> crateApiVaultBridgeMergeVaultFromFileHeld({
    required String path,
  });

  Future<MergeSummary> crateApiVaultBridgeMergeVaultFromFileWithKey({
    required String path,
    required List<int> passphrase,
    required List<int> hmacSecret,
    required List<int> credentialId,
  });

  Future<MergeSummary?> crateApiVaultBridgeMergeVaultFromFileWithKeyHeld({
    required String path,
    required List<int> hmacSecret,
    required List<int> credentialId,
  });

  Future<BigInt> crateApiVaultNowMs();

  Future<AssertionRequest> crateApiPasskeyBridgeParseAssertionRequest({
    required String json,
  });

  Future<CreationRequest> crateApiPasskeyBridgeParseCreationRequest({
    required String json,
  });

  Future<void> crateApiPasskeyDaemonBridgePasskeyDaemonOpen();

  Future<void> crateApiPasskeyDaemonBridgePasskeyDaemonRespond({
    required List<int> response,
  });

  Stream<Uint8List> crateApiPasskeyDaemonBridgePasskeyDaemonStart();

  Future<void> crateApiPasskeyDaemonBridgePasskeyDaemonStop();

  Future<Uint8List> crateApiPasskeyDaemonBridgePasskeyDenied();

  Future<Uint8List> crateApiPasskeyDaemonBridgePasskeyPerform({
    required List<int> payload,
    required BigInt accountIndex,
  });

  Future<PasskeyPlan> crateApiPasskeyDaemonBridgePasskeyPlan({
    required List<int> payload,
  });

  Future<List<PasskeyMatchData>> crateApiPasskeyBridgePasskeysForRequest({
    required String requestJson,
  });

  Future<double> crateApiPassphraseGeneratorPassphraseEntropyBits({
    required int wordCount,
    required Language language,
  });

  VaultHeaderData crateApiVaultBridgeReadVaultHeader({required String path});

  Future<RegistrationParts> crateApiPasskeyBridgeRegisterPasskey({
    required String requestJson,
  });

  Future<RegistrationParts> crateApiPasskeyBridgeRegisterPasskeyParts({
    required String rpId,
    required String userName,
    required String userDisplayName,
    required List<int> userHandle,
  });

  Future<String> crateApiPasskeyBridgeRegistrationResponseJson({
    required String requestJson,
    String? clientDataJsonB64Url,
  });

  Future<void> crateApiVaultBridgeRemoveAttachment({
    required String entryId,
    required String uuid,
  });

  Future<void> crateApiVaultBridgeRemoveYubikey({required List<int> credId});

  Future<void> crateApiVaultBridgeRenameFolder({
    required String oldName,
    required String newName,
  });

  Future<void> crateApiVaultBridgeRestoreHistory({
    required String id,
    required int index,
  });

  Future<void> crateApiVaultBridgeRestoreVaultBackup({required String path});

  Future<void> crateApiVaultBridgeRestoreVaultFromFile({
    required String path,
    required String source,
  });

  Future<void> crateApiSimpleSetProcessDumpable({required bool dumpable});

  Future<void> crateApiVaultBridgeSetVaultAlias({required String alias});

  Future<void> crateApiVaultBridgeSetYubikeyAlias({
    required String credentialIdHex,
    required String alias,
  });

  Future<AssertionParts> crateApiPasskeyBridgeSignPasskeyAssertion({
    required String entryId,
    required List<int> clientDataHash,
  });

  CsvPreviewData crateApiImportSniffCsvFile({required String input});

  Future<void> crateApiVaultBridgeUnlockVault({
    required List<int> passphrase,
    required String path,
  });

  Future<void> crateApiVaultBridgeUnlockVaultWithYubikey({
    required List<int> passphrase,
    required List<int> hmacSecret,
    required List<int> credentialId,
    required String path,
  });

  Future<void> crateApiVaultBridgeUpdateEntry({
    required VaultEntryData entry,
    int? expiryDays,
  });

  Future<bool> crateApiVaultBridgeVaultBackupUsable({required String path});

  Future<bool> crateApiVaultBridgeVaultFormatTooNew({required String path});

  Future<bool> crateApiVaultBridgeVaultFormatTooOld({required String path});
}

class RustLibApiImpl extends RustLibApiImplPlatform implements RustLibApi {
  RustLibApiImpl({
    required super.handler,
    required super.wire,
    required super.generalizedFrbRustBinding,
    required super.portManager,
  });

  @override
  Future<String> crateApiVaultBridgeAddAttachment({
    required String entryId,
    required String name,
    required String kind,
    required List<int> data,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(entryId, serializer);
          sse_encode_String(name, serializer);
          sse_encode_String(kind, serializer);
          sse_encode_list_prim_u_8_loose(data, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 1,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_String,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeAddAttachmentConstMeta,
        argValues: [entryId, name, kind, data],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeAddAttachmentConstMeta =>
      const TaskConstMeta(
        debugName: "add_attachment",
        argNames: ["entryId", "name", "kind", "data"],
      );

  @override
  Future<void> crateApiVaultBridgeAddYubikey({
    required List<int> newCredId,
    required List<int> newHmacSecret,
    required List<int> newSalt,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_list_prim_u_8_loose(newCredId, serializer);
          sse_encode_list_prim_u_8_loose(newHmacSecret, serializer);
          sse_encode_list_prim_u_8_loose(newSalt, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 2,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeAddYubikeyConstMeta,
        argValues: [newCredId, newHmacSecret, newSalt],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeAddYubikeyConstMeta =>
      const TaskConstMeta(
        debugName: "add_yubikey",
        argNames: ["newCredId", "newHmacSecret", "newSalt"],
      );

  @override
  Future<void> crateApiVaultBridgeAdoptVaultFile({
    required String source,
    required String dest,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(source, serializer);
          sse_encode_String(dest, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 3,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeAdoptVaultFileConstMeta,
        argValues: [source, dest],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeAdoptVaultFileConstMeta =>
      const TaskConstMeta(
        debugName: "adopt_vault_file",
        argNames: ["source", "dest"],
      );

  @override
  Future<void> crateApiVaultBridgeApplySyncDecisions({
    required List<SyncFieldResolutionInput> fieldResolutions,
    required List<SyncHistoryReplacementInput> historyReplacements,
    required List<SyncItemDeleteInput> itemDeletes,
    required List<SyncFolderInput> folders,
    required List<String> entryDeletes,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_list_sync_field_resolution_input(
            fieldResolutions,
            serializer,
          );
          sse_encode_list_sync_history_replacement_input(
            historyReplacements,
            serializer,
          );
          sse_encode_list_sync_item_delete_input(itemDeletes, serializer);
          sse_encode_list_sync_folder_input(folders, serializer);
          sse_encode_list_String(entryDeletes, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 4,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeApplySyncDecisionsConstMeta,
        argValues: [
          fieldResolutions,
          historyReplacements,
          itemDeletes,
          folders,
          entryDeletes,
        ],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeApplySyncDecisionsConstMeta =>
      const TaskConstMeta(
        debugName: "apply_sync_decisions",
        argNames: [
          "fieldResolutions",
          "historyReplacements",
          "itemDeletes",
          "folders",
          "entryDeletes",
        ],
      );

  @override
  Future<String> crateApiPasskeyBridgeAssertionResponseJson({
    required String entryId,
    String? clientDataJsonB64Url,
    String? clientDataHashB64Url,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(entryId, serializer);
          sse_encode_opt_String(clientDataJsonB64Url, serializer);
          sse_encode_opt_String(clientDataHashB64Url, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 5,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_String,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiPasskeyBridgeAssertionResponseJsonConstMeta,
        argValues: [entryId, clientDataJsonB64Url, clientDataHashB64Url],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiPasskeyBridgeAssertionResponseJsonConstMeta =>
      const TaskConstMeta(
        debugName: "assertion_response_json",
        argNames: ["entryId", "clientDataJsonB64Url", "clientDataHashB64Url"],
      );

  @override
  Future<void> crateApiVaultBridgeAssignFolderToEntries({
    required List<String> ids,
    required String folder,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_list_String(ids, serializer);
          sse_encode_String(folder, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 6,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeAssignFolderToEntriesConstMeta,
        argValues: [ids, folder],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeAssignFolderToEntriesConstMeta =>
      const TaskConstMeta(
        debugName: "assign_folder_to_entries",
        argNames: ["ids", "folder"],
      );

  @override
  Future<CapturedWindowData?>
  crateApiAutotypeBridgeAutotypeCaptureActiveWindow() {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 7,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_opt_box_autoadd_captured_window_data,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiAutotypeBridgeAutotypeCaptureActiveWindowConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta
  get kCrateApiAutotypeBridgeAutotypeCaptureActiveWindowConstMeta =>
      const TaskConstMeta(
        debugName: "autotype_capture_active_window",
        argNames: [],
      );

  @override
  Future<void> crateApiAutotypeBridgeAutotypeFill({
    required int windowId,
    required String entryId,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_u_32(windowId, serializer);
          sse_encode_String(entryId, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 8,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiAutotypeBridgeAutotypeFillConstMeta,
        argValues: [windowId, entryId],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiAutotypeBridgeAutotypeFillConstMeta =>
      const TaskConstMeta(
        debugName: "autotype_fill",
        argNames: ["windowId", "entryId"],
      );

  @override
  Future<String> crateApiAutotypeBridgeAutotypeSocketPath() {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 9,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_String,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiAutotypeBridgeAutotypeSocketPathConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiAutotypeBridgeAutotypeSocketPathConstMeta =>
      const TaskConstMeta(debugName: "autotype_socket_path", argNames: []);

  @override
  Future<String> crateApiAutotypeBridgeAutotypeTriggerToken() {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 10,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_String,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiAutotypeBridgeAutotypeTriggerTokenConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiAutotypeBridgeAutotypeTriggerTokenConstMeta =>
      const TaskConstMeta(debugName: "autotype_trigger_token", argNames: []);

  @override
  Future<ExportArtifact> crateApiVaultBridgeBuildExportBytes({
    required String vaultFilename,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(vaultFilename, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 11,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_export_artifact,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeBuildExportBytesConstMeta,
        argValues: [vaultFilename],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeBuildExportBytesConstMeta =>
      const TaskConstMeta(
        debugName: "build_export_bytes",
        argNames: ["vaultFilename"],
      );

  @override
  Future<ExportArtifact> crateApiVaultBridgeBuildExportPassphraseOnlyBytes({
    required String vaultFilename,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(vaultFilename, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 12,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_export_artifact,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeBuildExportPassphraseOnlyBytesConstMeta,
        argValues: [vaultFilename],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta
  get kCrateApiVaultBridgeBuildExportPassphraseOnlyBytesConstMeta =>
      const TaskConstMeta(
        debugName: "build_export_passphrase_only_bytes",
        argNames: ["vaultFilename"],
      );

  @override
  Future<void> crateApiVaultBridgeCancelSync() {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 13,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeCancelSyncConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeCancelSyncConstMeta =>
      const TaskConstMeta(debugName: "cancel_sync", argNames: []);

  @override
  Future<void> crateApiVaultBridgeChangePassphrase({
    required List<int> oldPassphrase,
    required List<int> newPassphrase,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_list_prim_u_8_loose(oldPassphrase, serializer);
          sse_encode_list_prim_u_8_loose(newPassphrase, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 14,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeChangePassphraseConstMeta,
        argValues: [oldPassphrase, newPassphrase],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeChangePassphraseConstMeta =>
      const TaskConstMeta(
        debugName: "change_passphrase",
        argNames: ["oldPassphrase", "newPassphrase"],
      );

  @override
  Future<String> crateApiVaultChronoNow() {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 15,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_String,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiVaultChronoNowConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultChronoNowConstMeta =>
      const TaskConstMeta(debugName: "chrono_now", argNames: []);

  @override
  Future<EntrySummaryData> crateApiVaultBridgeCreateEntry({
    required VaultEntryData entry,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_box_autoadd_vault_entry_data(entry, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 16,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_entry_summary_data,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeCreateEntryConstMeta,
        argValues: [entry],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeCreateEntryConstMeta =>
      const TaskConstMeta(debugName: "create_entry", argNames: ["entry"]);

  @override
  Future<void> crateApiVaultBridgeCreateFolder({required String name}) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(name, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 17,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeCreateFolderConstMeta,
        argValues: [name],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeCreateFolderConstMeta =>
      const TaskConstMeta(debugName: "create_folder", argNames: ["name"]);

  @override
  Future<LoginEntryData> crateApiVaultCreateLoginEntry({
    required String folder,
    required String title,
    required String url,
    required String username,
    required String password,
    String? notes,
    required List<CustomFieldData> customFields,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(folder, serializer);
          sse_encode_String(title, serializer);
          sse_encode_String(url, serializer);
          sse_encode_String(username, serializer);
          sse_encode_String(password, serializer);
          sse_encode_opt_String(notes, serializer);
          sse_encode_list_custom_field_data(customFields, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 18,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_login_entry_data,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiVaultCreateLoginEntryConstMeta,
        argValues: [
          folder,
          title,
          url,
          username,
          password,
          notes,
          customFields,
        ],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultCreateLoginEntryConstMeta =>
      const TaskConstMeta(
        debugName: "create_login_entry",
        argNames: [
          "folder",
          "title",
          "url",
          "username",
          "password",
          "notes",
          "customFields",
        ],
      );

  @override
  Future<void> crateApiVaultBridgeDeleteEntries({required List<String> ids}) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_list_String(ids, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 19,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeDeleteEntriesConstMeta,
        argValues: [ids],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeDeleteEntriesConstMeta =>
      const TaskConstMeta(debugName: "delete_entries", argNames: ["ids"]);

  @override
  Future<void> crateApiVaultBridgeDeleteEntry({required String id}) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(id, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 20,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeDeleteEntryConstMeta,
        argValues: [id],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeDeleteEntryConstMeta =>
      const TaskConstMeta(debugName: "delete_entry", argNames: ["id"]);

  @override
  Future<void> crateApiVaultBridgeDeleteFolder({
    required String name,
    String? reassignTo,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(name, serializer);
          sse_encode_opt_String(reassignTo, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 21,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeDeleteFolderConstMeta,
        argValues: [name, reassignTo],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeDeleteFolderConstMeta =>
      const TaskConstMeta(
        debugName: "delete_folder",
        argNames: ["name", "reassignTo"],
      );

  @override
  Future<void> crateApiVaultBridgeDeleteHistory({
    required String id,
    required int index,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(id, serializer);
          sse_encode_u_32(index, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 22,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeDeleteHistoryConstMeta,
        argValues: [id, index],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeDeleteHistoryConstMeta =>
      const TaskConstMeta(
        debugName: "delete_history",
        argNames: ["id", "index"],
      );

  @override
  Future<void> crateApiVaultBridgeDeleteWholeVault() {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 23,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeDeleteWholeVaultConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeDeleteWholeVaultConstMeta =>
      const TaskConstMeta(debugName: "delete_whole_vault", argNames: []);

  @override
  double crateApiPasswordGeneratorEntropyBits({
    required int poolSize,
    required int length,
  }) {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_u_32(poolSize, serializer);
          sse_encode_u_32(length, serializer);
          return pdeCallFfi(generalizedFrbRustBinding, serializer, funcId: 24)!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_f_64,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiPasswordGeneratorEntropyBitsConstMeta,
        argValues: [poolSize, length],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiPasswordGeneratorEntropyBitsConstMeta =>
      const TaskConstMeta(
        debugName: "entropy_bits",
        argNames: ["poolSize", "length"],
      );

  @override
  EntropyResult crateApiEntropyEstimateEntropy({required String password}) {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(password, serializer);
          return pdeCallFfi(generalizedFrbRustBinding, serializer, funcId: 25)!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_entropy_result,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiEntropyEstimateEntropyConstMeta,
        argValues: [password],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiEntropyEstimateEntropyConstMeta =>
      const TaskConstMeta(
        debugName: "estimate_entropy",
        argNames: ["password"],
      );

  @override
  Future<void> crateApiVaultBridgeExportVault({required String path}) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(path, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 26,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeExportVaultConstMeta,
        argValues: [path],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeExportVaultConstMeta =>
      const TaskConstMeta(debugName: "export_vault", argNames: ["path"]);

  @override
  Future<void> crateApiVaultBridgeExportVaultJson({required String path}) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(path, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 27,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeExportVaultJsonConstMeta,
        argValues: [path],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeExportVaultJsonConstMeta =>
      const TaskConstMeta(debugName: "export_vault_json", argNames: ["path"]);

  @override
  Future<void> crateApiVaultBridgeExportVaultPassphraseOnly({
    required String path,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(path, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 28,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeExportVaultPassphraseOnlyConstMeta,
        argValues: [path],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeExportVaultPassphraseOnlyConstMeta =>
      const TaskConstMeta(
        debugName: "export_vault_passphrase_only",
        argNames: ["path"],
      );

  @override
  Future<Uint8List> crateApiVaultBridgeExtractAttachment({
    required String entryId,
    required String uuid,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(entryId, serializer);
          sse_encode_String(uuid, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 29,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_prim_u_8_strict,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeExtractAttachmentConstMeta,
        argValues: [entryId, uuid],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeExtractAttachmentConstMeta =>
      const TaskConstMeta(
        debugName: "extract_attachment",
        argNames: ["entryId", "uuid"],
      );

  @override
  Future<MergeSummary> crateApiVaultBridgeFastMergeVaultFromFile({
    required String path,
    required List<int> passphrase,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(path, serializer);
          sse_encode_list_prim_u_8_loose(passphrase, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 30,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_merge_summary,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeFastMergeVaultFromFileConstMeta,
        argValues: [path, passphrase],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeFastMergeVaultFromFileConstMeta =>
      const TaskConstMeta(
        debugName: "fast_merge_vault_from_file",
        argNames: ["path", "passphrase"],
      );

  @override
  Future<MergeSummary?> crateApiVaultBridgeFastMergeVaultFromFileHeld({
    required String path,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(path, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 31,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_opt_box_autoadd_merge_summary,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeFastMergeVaultFromFileHeldConstMeta,
        argValues: [path],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeFastMergeVaultFromFileHeldConstMeta =>
      const TaskConstMeta(
        debugName: "fast_merge_vault_from_file_held",
        argNames: ["path"],
      );

  @override
  Future<MergeSummary> crateApiVaultBridgeFastMergeVaultFromFileWithKey({
    required String path,
    required List<int> passphrase,
    required List<int> hmacSecret,
    required List<int> credentialId,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(path, serializer);
          sse_encode_list_prim_u_8_loose(passphrase, serializer);
          sse_encode_list_prim_u_8_loose(hmacSecret, serializer);
          sse_encode_list_prim_u_8_loose(credentialId, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 32,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_merge_summary,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeFastMergeVaultFromFileWithKeyConstMeta,
        argValues: [path, passphrase, hmacSecret, credentialId],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta
  get kCrateApiVaultBridgeFastMergeVaultFromFileWithKeyConstMeta =>
      const TaskConstMeta(
        debugName: "fast_merge_vault_from_file_with_key",
        argNames: ["path", "passphrase", "hmacSecret", "credentialId"],
      );

  @override
  Future<MergeSummary?> crateApiVaultBridgeFastMergeVaultFromFileWithKeyHeld({
    required String path,
    required List<int> hmacSecret,
    required List<int> credentialId,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(path, serializer);
          sse_encode_list_prim_u_8_loose(hmacSecret, serializer);
          sse_encode_list_prim_u_8_loose(credentialId, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 33,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_opt_box_autoadd_merge_summary,
          decodeErrorData: sse_decode_String,
        ),
        constMeta:
            kCrateApiVaultBridgeFastMergeVaultFromFileWithKeyHeldConstMeta,
        argValues: [path, hmacSecret, credentialId],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta
  get kCrateApiVaultBridgeFastMergeVaultFromFileWithKeyHeldConstMeta =>
      const TaskConstMeta(
        debugName: "fast_merge_vault_from_file_with_key_held",
        argNames: ["path", "hmacSecret", "credentialId"],
      );

  @override
  Future<Uint8List> crateApiFidoBridgeFidoGetHmacSecret({
    required String devicePath,
    required List<int> credentialId,
    required List<int> salt,
    required String pin,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(devicePath, serializer);
          sse_encode_list_prim_u_8_loose(credentialId, serializer);
          sse_encode_list_prim_u_8_loose(salt, serializer);
          sse_encode_String(pin, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 34,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_prim_u_8_strict,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiFidoBridgeFidoGetHmacSecretConstMeta,
        argValues: [devicePath, credentialId, salt, pin],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiFidoBridgeFidoGetHmacSecretConstMeta =>
      const TaskConstMeta(
        debugName: "fido_get_hmac_secret",
        argNames: ["devicePath", "credentialId", "salt", "pin"],
      );

  @override
  Future<FidoHmacMatch> crateApiFidoBridgeFidoGetHmacSecretAny({
    required String devicePath,
    required List<FidoRecordInput> records,
    required String pin,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(devicePath, serializer);
          sse_encode_list_fido_record_input(records, serializer);
          sse_encode_String(pin, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 35,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_fido_hmac_match,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiFidoBridgeFidoGetHmacSecretAnyConstMeta,
        argValues: [devicePath, records, pin],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiFidoBridgeFidoGetHmacSecretAnyConstMeta =>
      const TaskConstMeta(
        debugName: "fido_get_hmac_secret_any",
        argNames: ["devicePath", "records", "pin"],
      );

  @override
  List<String> crateApiFidoBridgeFidoListDevices() {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          return pdeCallFfi(generalizedFrbRustBinding, serializer, funcId: 36)!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_String,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiFidoBridgeFidoListDevicesConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiFidoBridgeFidoListDevicesConstMeta =>
      const TaskConstMeta(debugName: "fido_list_devices", argNames: []);

  @override
  Future<FidoCredentialData> crateApiFidoBridgeFidoRegister({
    required String devicePath,
    required String pin,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(devicePath, serializer);
          sse_encode_String(pin, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 37,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_fido_credential_data,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiFidoBridgeFidoRegisterConstMeta,
        argValues: [devicePath, pin],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiFidoBridgeFidoRegisterConstMeta =>
      const TaskConstMeta(
        debugName: "fido_register",
        argNames: ["devicePath", "pin"],
      );

  @override
  Future<String> crateApiPassphraseGeneratorGeneratePassphrase({
    required PassphraseConfig config,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_box_autoadd_passphrase_config(config, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 38,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_String,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiPassphraseGeneratorGeneratePassphraseConstMeta,
        argValues: [config],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiPassphraseGeneratorGeneratePassphraseConstMeta =>
      const TaskConstMeta(
        debugName: "generate_passphrase",
        argNames: ["config"],
      );

  @override
  String crateApiPasswordGeneratorGeneratePassword({
    required PasswordConfig config,
  }) {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_box_autoadd_password_config(config, serializer);
          return pdeCallFfi(generalizedFrbRustBinding, serializer, funcId: 39)!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_String,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiPasswordGeneratorGeneratePasswordConstMeta,
        argValues: [config],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiPasswordGeneratorGeneratePasswordConstMeta =>
      const TaskConstMeta(debugName: "generate_password", argNames: ["config"]);

  @override
  VaultEntryData crateApiVaultBridgeGetEntry({required String id}) {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(id, serializer);
          return pdeCallFfi(generalizedFrbRustBinding, serializer, funcId: 40)!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_vault_entry_data,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeGetEntryConstMeta,
        argValues: [id],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeGetEntryConstMeta =>
      const TaskConstMeta(debugName: "get_entry", argNames: ["id"]);

  @override
  Future<List<HistoryRecordData>> crateApiVaultBridgeGetEntryHistory({
    required String id,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(id, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 41,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_history_record_data,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeGetEntryHistoryConstMeta,
        argValues: [id],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeGetEntryHistoryConstMeta =>
      const TaskConstMeta(debugName: "get_entry_history", argNames: ["id"]);

  @override
  Future<ImportResult> crateApiImportImportFromBitwarden({
    required List<int> data,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_list_prim_u_8_loose(data, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 42,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_import_result,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiImportImportFromBitwardenConstMeta,
        argValues: [data],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiImportImportFromBitwardenConstMeta =>
      const TaskConstMeta(
        debugName: "import_from_bitwarden",
        argNames: ["data"],
      );

  @override
  Future<ImportResult> crateApiImportImportFromCsv({
    required String input,
    required CsvImportConfigData config,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(input, serializer);
          sse_encode_box_autoadd_csv_import_config_data(config, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 43,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_import_result,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiImportImportFromCsvConstMeta,
        argValues: [input, config],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiImportImportFromCsvConstMeta =>
      const TaskConstMeta(
        debugName: "import_from_csv",
        argNames: ["input", "config"],
      );

  @override
  Future<ImportResult> crateApiImportImportFromDashlane({
    required List<int> data,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_list_prim_u_8_loose(data, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 44,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_import_result,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiImportImportFromDashlaneConstMeta,
        argValues: [data],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiImportImportFromDashlaneConstMeta =>
      const TaskConstMeta(
        debugName: "import_from_dashlane",
        argNames: ["data"],
      );

  @override
  Future<ImportResult> crateApiImportImportFromEnpass({
    required List<int> data,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_list_prim_u_8_loose(data, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 45,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_import_result,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiImportImportFromEnpassConstMeta,
        argValues: [data],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiImportImportFromEnpassConstMeta =>
      const TaskConstMeta(debugName: "import_from_enpass", argNames: ["data"]);

  @override
  Future<GabbroImportResult> crateApiImportImportFromGabbro({
    required String path,
    required List<int> passphrase,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(path, serializer);
          sse_encode_list_prim_u_8_loose(passphrase, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 46,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_gabbro_import_result,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiImportImportFromGabbroConstMeta,
        argValues: [path, passphrase],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiImportImportFromGabbroConstMeta =>
      const TaskConstMeta(
        debugName: "import_from_gabbro",
        argNames: ["path", "passphrase"],
      );

  @override
  Future<GabbroImportResult> crateApiImportImportFromGabbroWithKey({
    required String path,
    required List<int> passphrase,
    required List<int> hmacSecret,
    required List<int> credentialId,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(path, serializer);
          sse_encode_list_prim_u_8_loose(passphrase, serializer);
          sse_encode_list_prim_u_8_loose(hmacSecret, serializer);
          sse_encode_list_prim_u_8_loose(credentialId, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 47,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_gabbro_import_result,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiImportImportFromGabbroWithKeyConstMeta,
        argValues: [path, passphrase, hmacSecret, credentialId],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiImportImportFromGabbroWithKeyConstMeta =>
      const TaskConstMeta(
        debugName: "import_from_gabbro_with_key",
        argNames: ["path", "passphrase", "hmacSecret", "credentialId"],
      );

  @override
  Future<ImportResult> crateApiImportImportFromGooglePm({
    required List<int> data,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_list_prim_u_8_loose(data, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 48,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_import_result,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiImportImportFromGooglePmConstMeta,
        argValues: [data],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiImportImportFromGooglePmConstMeta =>
      const TaskConstMeta(
        debugName: "import_from_google_pm",
        argNames: ["data"],
      );

  @override
  Future<void> crateApiSimpleInitApp() {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 49,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiSimpleInitAppConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiSimpleInitAppConstMeta =>
      const TaskConstMeta(debugName: "init_app", argNames: []);

  @override
  Future<void> crateApiVaultBridgeInitVault({
    required List<int> passphrase,
    required String path,
    String? alias,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_list_prim_u_8_loose(passphrase, serializer);
          sse_encode_String(path, serializer);
          sse_encode_opt_String(alias, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 50,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeInitVaultConstMeta,
        argValues: [passphrase, path, alias],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeInitVaultConstMeta =>
      const TaskConstMeta(
        debugName: "init_vault",
        argNames: ["passphrase", "path", "alias"],
      );

  @override
  Future<void> crateApiVaultBridgeInitVaultWithKeys({
    required List<int> passphrase,
    required List<YubiKeyInitData> keys,
    required String path,
    String? alias,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_list_prim_u_8_loose(passphrase, serializer);
          sse_encode_list_yubi_key_init_data(keys, serializer);
          sse_encode_String(path, serializer);
          sse_encode_opt_String(alias, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 51,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeInitVaultWithKeysConstMeta,
        argValues: [passphrase, keys, path, alias],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeInitVaultWithKeysConstMeta =>
      const TaskConstMeta(
        debugName: "init_vault_with_keys",
        argNames: ["passphrase", "keys", "path", "alias"],
      );

  @override
  List<EntrySummaryData> crateApiVaultBridgeListEntrySummaries() {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          return pdeCallFfi(generalizedFrbRustBinding, serializer, funcId: 52)!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_entry_summary_data,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeListEntrySummariesConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeListEntrySummariesConstMeta =>
      const TaskConstMeta(debugName: "list_entry_summaries", argNames: []);

  @override
  List<String> crateApiVaultBridgeListFolders() {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          return pdeCallFfi(generalizedFrbRustBinding, serializer, funcId: 53)!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_String,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeListFoldersConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeListFoldersConstMeta =>
      const TaskConstMeta(debugName: "list_folders", argNames: []);

  @override
  List<YubikeyRecordData> crateApiVaultBridgeListVaultYubikeyRecords({
    required String path,
  }) {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(path, serializer);
          return pdeCallFfi(generalizedFrbRustBinding, serializer, funcId: 54)!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_yubikey_record_data,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeListVaultYubikeyRecordsConstMeta,
        argValues: [path],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeListVaultYubikeyRecordsConstMeta =>
      const TaskConstMeta(
        debugName: "list_vault_yubikey_records",
        argNames: ["path"],
      );

  @override
  List<YubikeyAliasData> crateApiVaultBridgeListYubikeyAliases() {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          return pdeCallFfi(generalizedFrbRustBinding, serializer, funcId: 55)!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_yubikey_alias_data,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeListYubikeyAliasesConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeListYubikeyAliasesConstMeta =>
      const TaskConstMeta(debugName: "list_yubikey_aliases", argNames: []);

  @override
  void crateApiVaultBridgeLockVault() {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          return pdeCallFfi(generalizedFrbRustBinding, serializer, funcId: 56)!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeLockVaultConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeLockVaultConstMeta =>
      const TaskConstMeta(debugName: "lock_vault", argNames: []);

  @override
  Future<MergeSummary> crateApiVaultBridgeMergeVaultFromFile({
    required String path,
    required List<int> passphrase,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(path, serializer);
          sse_encode_list_prim_u_8_loose(passphrase, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 57,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_merge_summary,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeMergeVaultFromFileConstMeta,
        argValues: [path, passphrase],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeMergeVaultFromFileConstMeta =>
      const TaskConstMeta(
        debugName: "merge_vault_from_file",
        argNames: ["path", "passphrase"],
      );

  @override
  Future<MergeSummary?> crateApiVaultBridgeMergeVaultFromFileHeld({
    required String path,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(path, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 58,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_opt_box_autoadd_merge_summary,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeMergeVaultFromFileHeldConstMeta,
        argValues: [path],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeMergeVaultFromFileHeldConstMeta =>
      const TaskConstMeta(
        debugName: "merge_vault_from_file_held",
        argNames: ["path"],
      );

  @override
  Future<MergeSummary> crateApiVaultBridgeMergeVaultFromFileWithKey({
    required String path,
    required List<int> passphrase,
    required List<int> hmacSecret,
    required List<int> credentialId,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(path, serializer);
          sse_encode_list_prim_u_8_loose(passphrase, serializer);
          sse_encode_list_prim_u_8_loose(hmacSecret, serializer);
          sse_encode_list_prim_u_8_loose(credentialId, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 59,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_merge_summary,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeMergeVaultFromFileWithKeyConstMeta,
        argValues: [path, passphrase, hmacSecret, credentialId],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeMergeVaultFromFileWithKeyConstMeta =>
      const TaskConstMeta(
        debugName: "merge_vault_from_file_with_key",
        argNames: ["path", "passphrase", "hmacSecret", "credentialId"],
      );

  @override
  Future<MergeSummary?> crateApiVaultBridgeMergeVaultFromFileWithKeyHeld({
    required String path,
    required List<int> hmacSecret,
    required List<int> credentialId,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(path, serializer);
          sse_encode_list_prim_u_8_loose(hmacSecret, serializer);
          sse_encode_list_prim_u_8_loose(credentialId, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 60,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_opt_box_autoadd_merge_summary,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeMergeVaultFromFileWithKeyHeldConstMeta,
        argValues: [path, hmacSecret, credentialId],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta
  get kCrateApiVaultBridgeMergeVaultFromFileWithKeyHeldConstMeta =>
      const TaskConstMeta(
        debugName: "merge_vault_from_file_with_key_held",
        argNames: ["path", "hmacSecret", "credentialId"],
      );

  @override
  Future<BigInt> crateApiVaultNowMs() {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 61,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_u_64,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiVaultNowMsConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultNowMsConstMeta =>
      const TaskConstMeta(debugName: "now_ms", argNames: []);

  @override
  Future<AssertionRequest> crateApiPasskeyBridgeParseAssertionRequest({
    required String json,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(json, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 62,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_assertion_request,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiPasskeyBridgeParseAssertionRequestConstMeta,
        argValues: [json],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiPasskeyBridgeParseAssertionRequestConstMeta =>
      const TaskConstMeta(
        debugName: "parse_assertion_request",
        argNames: ["json"],
      );

  @override
  Future<CreationRequest> crateApiPasskeyBridgeParseCreationRequest({
    required String json,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(json, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 63,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_creation_request,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiPasskeyBridgeParseCreationRequestConstMeta,
        argValues: [json],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiPasskeyBridgeParseCreationRequestConstMeta =>
      const TaskConstMeta(
        debugName: "parse_creation_request",
        argNames: ["json"],
      );

  @override
  Future<void> crateApiPasskeyDaemonBridgePasskeyDaemonOpen() {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 64,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiPasskeyDaemonBridgePasskeyDaemonOpenConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiPasskeyDaemonBridgePasskeyDaemonOpenConstMeta =>
      const TaskConstMeta(debugName: "passkey_daemon_open", argNames: []);

  @override
  Future<void> crateApiPasskeyDaemonBridgePasskeyDaemonRespond({
    required List<int> response,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_list_prim_u_8_loose(response, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 65,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiPasskeyDaemonBridgePasskeyDaemonRespondConstMeta,
        argValues: [response],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiPasskeyDaemonBridgePasskeyDaemonRespondConstMeta =>
      const TaskConstMeta(
        debugName: "passkey_daemon_respond",
        argNames: ["response"],
      );

  @override
  Stream<Uint8List> crateApiPasskeyDaemonBridgePasskeyDaemonStart() {
    final sink = RustStreamSink<Uint8List>();
    unawaited(
      handler.executeNormal(
        NormalTask(
          callFfi: (port_) {
            final serializer = SseSerializer(generalizedFrbRustBinding);
            sse_encode_StreamSink_list_prim_u_8_strict_Sse(sink, serializer);
            pdeCallFfi(
              generalizedFrbRustBinding,
              serializer,
              funcId: 66,
              port: port_,
            );
          },
          codec: SseCodec(
            decodeSuccessData: sse_decode_unit,
            decodeErrorData: sse_decode_String,
          ),
          constMeta: kCrateApiPasskeyDaemonBridgePasskeyDaemonStartConstMeta,
          argValues: [sink],
          apiImpl: this,
        ),
      ),
    );
    return sink.stream;
  }

  TaskConstMeta get kCrateApiPasskeyDaemonBridgePasskeyDaemonStartConstMeta =>
      const TaskConstMeta(
        debugName: "passkey_daemon_start",
        argNames: ["sink"],
      );

  @override
  Future<void> crateApiPasskeyDaemonBridgePasskeyDaemonStop() {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 67,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiPasskeyDaemonBridgePasskeyDaemonStopConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiPasskeyDaemonBridgePasskeyDaemonStopConstMeta =>
      const TaskConstMeta(debugName: "passkey_daemon_stop", argNames: []);

  @override
  Future<Uint8List> crateApiPasskeyDaemonBridgePasskeyDenied() {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 68,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_prim_u_8_strict,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiPasskeyDaemonBridgePasskeyDeniedConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiPasskeyDaemonBridgePasskeyDeniedConstMeta =>
      const TaskConstMeta(debugName: "passkey_denied", argNames: []);

  @override
  Future<Uint8List> crateApiPasskeyDaemonBridgePasskeyPerform({
    required List<int> payload,
    required BigInt accountIndex,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_list_prim_u_8_loose(payload, serializer);
          sse_encode_usize(accountIndex, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 69,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_prim_u_8_strict,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiPasskeyDaemonBridgePasskeyPerformConstMeta,
        argValues: [payload, accountIndex],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiPasskeyDaemonBridgePasskeyPerformConstMeta =>
      const TaskConstMeta(
        debugName: "passkey_perform",
        argNames: ["payload", "accountIndex"],
      );

  @override
  Future<PasskeyPlan> crateApiPasskeyDaemonBridgePasskeyPlan({
    required List<int> payload,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_list_prim_u_8_loose(payload, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 70,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_passkey_plan,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiPasskeyDaemonBridgePasskeyPlanConstMeta,
        argValues: [payload],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiPasskeyDaemonBridgePasskeyPlanConstMeta =>
      const TaskConstMeta(debugName: "passkey_plan", argNames: ["payload"]);

  @override
  Future<List<PasskeyMatchData>> crateApiPasskeyBridgePasskeysForRequest({
    required String requestJson,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(requestJson, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 71,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_passkey_match_data,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiPasskeyBridgePasskeysForRequestConstMeta,
        argValues: [requestJson],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiPasskeyBridgePasskeysForRequestConstMeta =>
      const TaskConstMeta(
        debugName: "passkeys_for_request",
        argNames: ["requestJson"],
      );

  @override
  Future<double> crateApiPassphraseGeneratorPassphraseEntropyBits({
    required int wordCount,
    required Language language,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_u_32(wordCount, serializer);
          sse_encode_language(language, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 72,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_f_64,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiPassphraseGeneratorPassphraseEntropyBitsConstMeta,
        argValues: [wordCount, language],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta
  get kCrateApiPassphraseGeneratorPassphraseEntropyBitsConstMeta =>
      const TaskConstMeta(
        debugName: "passphrase_entropy_bits",
        argNames: ["wordCount", "language"],
      );

  @override
  VaultHeaderData crateApiVaultBridgeReadVaultHeader({required String path}) {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(path, serializer);
          return pdeCallFfi(generalizedFrbRustBinding, serializer, funcId: 73)!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_vault_header_data,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeReadVaultHeaderConstMeta,
        argValues: [path],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeReadVaultHeaderConstMeta =>
      const TaskConstMeta(debugName: "read_vault_header", argNames: ["path"]);

  @override
  Future<RegistrationParts> crateApiPasskeyBridgeRegisterPasskey({
    required String requestJson,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(requestJson, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 74,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_registration_parts,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiPasskeyBridgeRegisterPasskeyConstMeta,
        argValues: [requestJson],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiPasskeyBridgeRegisterPasskeyConstMeta =>
      const TaskConstMeta(
        debugName: "register_passkey",
        argNames: ["requestJson"],
      );

  @override
  Future<RegistrationParts> crateApiPasskeyBridgeRegisterPasskeyParts({
    required String rpId,
    required String userName,
    required String userDisplayName,
    required List<int> userHandle,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(rpId, serializer);
          sse_encode_String(userName, serializer);
          sse_encode_String(userDisplayName, serializer);
          sse_encode_list_prim_u_8_loose(userHandle, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 75,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_registration_parts,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiPasskeyBridgeRegisterPasskeyPartsConstMeta,
        argValues: [rpId, userName, userDisplayName, userHandle],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiPasskeyBridgeRegisterPasskeyPartsConstMeta =>
      const TaskConstMeta(
        debugName: "register_passkey_parts",
        argNames: ["rpId", "userName", "userDisplayName", "userHandle"],
      );

  @override
  Future<String> crateApiPasskeyBridgeRegistrationResponseJson({
    required String requestJson,
    String? clientDataJsonB64Url,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(requestJson, serializer);
          sse_encode_opt_String(clientDataJsonB64Url, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 76,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_String,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiPasskeyBridgeRegistrationResponseJsonConstMeta,
        argValues: [requestJson, clientDataJsonB64Url],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiPasskeyBridgeRegistrationResponseJsonConstMeta =>
      const TaskConstMeta(
        debugName: "registration_response_json",
        argNames: ["requestJson", "clientDataJsonB64Url"],
      );

  @override
  Future<void> crateApiVaultBridgeRemoveAttachment({
    required String entryId,
    required String uuid,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(entryId, serializer);
          sse_encode_String(uuid, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 77,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeRemoveAttachmentConstMeta,
        argValues: [entryId, uuid],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeRemoveAttachmentConstMeta =>
      const TaskConstMeta(
        debugName: "remove_attachment",
        argNames: ["entryId", "uuid"],
      );

  @override
  Future<void> crateApiVaultBridgeRemoveYubikey({required List<int> credId}) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_list_prim_u_8_loose(credId, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 78,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeRemoveYubikeyConstMeta,
        argValues: [credId],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeRemoveYubikeyConstMeta =>
      const TaskConstMeta(debugName: "remove_yubikey", argNames: ["credId"]);

  @override
  Future<void> crateApiVaultBridgeRenameFolder({
    required String oldName,
    required String newName,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(oldName, serializer);
          sse_encode_String(newName, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 79,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeRenameFolderConstMeta,
        argValues: [oldName, newName],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeRenameFolderConstMeta =>
      const TaskConstMeta(
        debugName: "rename_folder",
        argNames: ["oldName", "newName"],
      );

  @override
  Future<void> crateApiVaultBridgeRestoreHistory({
    required String id,
    required int index,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(id, serializer);
          sse_encode_u_32(index, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 80,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeRestoreHistoryConstMeta,
        argValues: [id, index],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeRestoreHistoryConstMeta =>
      const TaskConstMeta(
        debugName: "restore_history",
        argNames: ["id", "index"],
      );

  @override
  Future<void> crateApiVaultBridgeRestoreVaultBackup({required String path}) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(path, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 81,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeRestoreVaultBackupConstMeta,
        argValues: [path],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeRestoreVaultBackupConstMeta =>
      const TaskConstMeta(
        debugName: "restore_vault_backup",
        argNames: ["path"],
      );

  @override
  Future<void> crateApiVaultBridgeRestoreVaultFromFile({
    required String path,
    required String source,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(path, serializer);
          sse_encode_String(source, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 82,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeRestoreVaultFromFileConstMeta,
        argValues: [path, source],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeRestoreVaultFromFileConstMeta =>
      const TaskConstMeta(
        debugName: "restore_vault_from_file",
        argNames: ["path", "source"],
      );

  @override
  Future<void> crateApiSimpleSetProcessDumpable({required bool dumpable}) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_bool(dumpable, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 83,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiSimpleSetProcessDumpableConstMeta,
        argValues: [dumpable],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiSimpleSetProcessDumpableConstMeta =>
      const TaskConstMeta(
        debugName: "set_process_dumpable",
        argNames: ["dumpable"],
      );

  @override
  Future<void> crateApiVaultBridgeSetVaultAlias({required String alias}) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(alias, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 84,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeSetVaultAliasConstMeta,
        argValues: [alias],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeSetVaultAliasConstMeta =>
      const TaskConstMeta(debugName: "set_vault_alias", argNames: ["alias"]);

  @override
  Future<void> crateApiVaultBridgeSetYubikeyAlias({
    required String credentialIdHex,
    required String alias,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(credentialIdHex, serializer);
          sse_encode_String(alias, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 85,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeSetYubikeyAliasConstMeta,
        argValues: [credentialIdHex, alias],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeSetYubikeyAliasConstMeta =>
      const TaskConstMeta(
        debugName: "set_yubikey_alias",
        argNames: ["credentialIdHex", "alias"],
      );

  @override
  Future<AssertionParts> crateApiPasskeyBridgeSignPasskeyAssertion({
    required String entryId,
    required List<int> clientDataHash,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(entryId, serializer);
          sse_encode_list_prim_u_8_loose(clientDataHash, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 86,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_assertion_parts,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiPasskeyBridgeSignPasskeyAssertionConstMeta,
        argValues: [entryId, clientDataHash],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiPasskeyBridgeSignPasskeyAssertionConstMeta =>
      const TaskConstMeta(
        debugName: "sign_passkey_assertion",
        argNames: ["entryId", "clientDataHash"],
      );

  @override
  CsvPreviewData crateApiImportSniffCsvFile({required String input}) {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(input, serializer);
          return pdeCallFfi(generalizedFrbRustBinding, serializer, funcId: 87)!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_csv_preview_data,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiImportSniffCsvFileConstMeta,
        argValues: [input],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiImportSniffCsvFileConstMeta =>
      const TaskConstMeta(debugName: "sniff_csv_file", argNames: ["input"]);

  @override
  Future<void> crateApiVaultBridgeUnlockVault({
    required List<int> passphrase,
    required String path,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_list_prim_u_8_loose(passphrase, serializer);
          sse_encode_String(path, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 88,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeUnlockVaultConstMeta,
        argValues: [passphrase, path],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeUnlockVaultConstMeta =>
      const TaskConstMeta(
        debugName: "unlock_vault",
        argNames: ["passphrase", "path"],
      );

  @override
  Future<void> crateApiVaultBridgeUnlockVaultWithYubikey({
    required List<int> passphrase,
    required List<int> hmacSecret,
    required List<int> credentialId,
    required String path,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_list_prim_u_8_loose(passphrase, serializer);
          sse_encode_list_prim_u_8_loose(hmacSecret, serializer);
          sse_encode_list_prim_u_8_loose(credentialId, serializer);
          sse_encode_String(path, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 89,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeUnlockVaultWithYubikeyConstMeta,
        argValues: [passphrase, hmacSecret, credentialId, path],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeUnlockVaultWithYubikeyConstMeta =>
      const TaskConstMeta(
        debugName: "unlock_vault_with_yubikey",
        argNames: ["passphrase", "hmacSecret", "credentialId", "path"],
      );

  @override
  Future<void> crateApiVaultBridgeUpdateEntry({
    required VaultEntryData entry,
    int? expiryDays,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_box_autoadd_vault_entry_data(entry, serializer);
          sse_encode_opt_box_autoadd_u_32(expiryDays, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 90,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeUpdateEntryConstMeta,
        argValues: [entry, expiryDays],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeUpdateEntryConstMeta =>
      const TaskConstMeta(
        debugName: "update_entry",
        argNames: ["entry", "expiryDays"],
      );

  @override
  Future<bool> crateApiVaultBridgeVaultBackupUsable({required String path}) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(path, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 91,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_bool,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiVaultBridgeVaultBackupUsableConstMeta,
        argValues: [path],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeVaultBackupUsableConstMeta =>
      const TaskConstMeta(debugName: "vault_backup_usable", argNames: ["path"]);

  @override
  Future<bool> crateApiVaultBridgeVaultFormatTooNew({required String path}) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(path, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 92,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_bool,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeVaultFormatTooNewConstMeta,
        argValues: [path],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeVaultFormatTooNewConstMeta =>
      const TaskConstMeta(
        debugName: "vault_format_too_new",
        argNames: ["path"],
      );

  @override
  Future<bool> crateApiVaultBridgeVaultFormatTooOld({required String path}) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(path, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 93,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_bool,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiVaultBridgeVaultFormatTooOldConstMeta,
        argValues: [path],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiVaultBridgeVaultFormatTooOldConstMeta =>
      const TaskConstMeta(
        debugName: "vault_format_too_old",
        argNames: ["path"],
      );

  @protected
  AnyhowException dco_decode_AnyhowException(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return AnyhowException(raw as String);
  }

  @protected
  RustStreamSink<Uint8List> dco_decode_StreamSink_list_prim_u_8_strict_Sse(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    throw UnimplementedError();
  }

  @protected
  String dco_decode_String(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw as String;
  }

  @protected
  AddedEntryItem dco_decode_added_entry_item(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2)
      throw Exception('unexpected arr length: expect 2 but see ${arr.length}');
    return AddedEntryItem(
      id: dco_decode_String(arr[0]),
      title: dco_decode_String(arr[1]),
    );
  }

  @protected
  AssertionParts dco_decode_assertion_parts(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 4)
      throw Exception('unexpected arr length: expect 4 but see ${arr.length}');
    return AssertionParts(
      credentialId: dco_decode_list_prim_u_8_strict(arr[0]),
      authData: dco_decode_list_prim_u_8_strict(arr[1]),
      signatureDer: dco_decode_list_prim_u_8_strict(arr[2]),
      userHandle: dco_decode_list_prim_u_8_strict(arr[3]),
    );
  }

  @protected
  AssertionRequest dco_decode_assertion_request(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 3)
      throw Exception('unexpected arr length: expect 3 but see ${arr.length}');
    return AssertionRequest(
      rpId: dco_decode_String(arr[0]),
      challenge: dco_decode_list_prim_u_8_strict(arr[1]),
      allowCredentialIds: dco_decode_list_list_prim_u_8_strict(arr[2]),
    );
  }

  @protected
  AttachmentMetaData dco_decode_attachment_meta_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 4)
      throw Exception('unexpected arr length: expect 4 but see ${arr.length}');
    return AttachmentMetaData(
      uuid: dco_decode_String(arr[0]),
      name: dco_decode_String(arr[1]),
      kind: dco_decode_String(arr[2]),
      size: dco_decode_u_64(arr[3]),
    );
  }

  @protected
  bool dco_decode_bool(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw as bool;
  }

  @protected
  CapturedWindowData dco_decode_box_autoadd_captured_window_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_captured_window_data(raw);
  }

  @protected
  CardEntryData dco_decode_box_autoadd_card_entry_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_card_entry_data(raw);
  }

  @protected
  CsvImportConfigData dco_decode_box_autoadd_csv_import_config_data(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_csv_import_config_data(raw);
  }

  @protected
  CustomEntryData dco_decode_box_autoadd_custom_entry_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_custom_entry_data(raw);
  }

  @protected
  FileEntryData dco_decode_box_autoadd_file_entry_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_file_entry_data(raw);
  }

  @protected
  IdentityEntryData dco_decode_box_autoadd_identity_entry_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_identity_entry_data(raw);
  }

  @protected
  LoginEntryData dco_decode_box_autoadd_login_entry_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_login_entry_data(raw);
  }

  @protected
  MergeSummary dco_decode_box_autoadd_merge_summary(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_merge_summary(raw);
  }

  @protected
  NoteEntryData dco_decode_box_autoadd_note_entry_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_note_entry_data(raw);
  }

  @protected
  PasskeyEntryData dco_decode_box_autoadd_passkey_entry_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_passkey_entry_data(raw);
  }

  @protected
  PassphraseConfig dco_decode_box_autoadd_passphrase_config(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_passphrase_config(raw);
  }

  @protected
  PasswordConfig dco_decode_box_autoadd_password_config(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_password_config(raw);
  }

  @protected
  int dco_decode_box_autoadd_u_32(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw as int;
  }

  @protected
  VaultEntryData dco_decode_box_autoadd_vault_entry_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_vault_entry_data(raw);
  }

  @protected
  BroughtOverItem dco_decode_brought_over_item(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 5)
      throw Exception('unexpected arr length: expect 5 but see ${arr.length}');
    return BroughtOverItem(
      id: dco_decode_String(arr[0]),
      title: dco_decode_String(arr[1]),
      field: dco_decode_String(arr[2]),
      oldValue: dco_decode_String(arr[3]),
      newValue: dco_decode_String(arr[4]),
    );
  }

  @protected
  CapturedWindowData dco_decode_captured_window_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 3)
      throw Exception('unexpected arr length: expect 3 but see ${arr.length}');
    return CapturedWindowData(
      id: dco_decode_u_32(arr[0]),
      appClass: dco_decode_String(arr[1]),
      title: dco_decode_String(arr[2]),
    );
  }

  @protected
  CardEntryData dco_decode_card_entry_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 19)
      throw Exception('unexpected arr length: expect 19 but see ${arr.length}');
    return CardEntryData(
      id: dco_decode_String(arr[0]),
      createdAt: dco_decode_String(arr[1]),
      updatedAt: dco_decode_String(arr[2]),
      folder: dco_decode_String(arr[3]),
      cardName: dco_decode_opt_String(arr[4]),
      status: dco_decode_String(arr[5]),
      cardholderName: dco_decode_String(arr[6]),
      cardNumber: dco_decode_String(arr[7]),
      expiry: dco_decode_String(arr[8]),
      cvv: dco_decode_String(arr[9]),
      creditLimit: dco_decode_opt_String(arr[10]),
      cardAccountNumber: dco_decode_opt_String(arr[11]),
      paymentNetwork: dco_decode_opt_String(arr[12]),
      pin: dco_decode_opt_String(arr[13]),
      bankName: dco_decode_opt_String(arr[14]),
      transactionPassword: dco_decode_opt_String(arr[15]),
      notes: dco_decode_opt_String(arr[16]),
      customFields: dco_decode_list_custom_field_data(arr[17]),
      attachments: dco_decode_list_attachment_meta_data(arr[18]),
    );
  }

  @protected
  CreationRequest dco_decode_creation_request(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 5)
      throw Exception('unexpected arr length: expect 5 but see ${arr.length}');
    return CreationRequest(
      rpId: dco_decode_String(arr[0]),
      userHandle: dco_decode_list_prim_u_8_strict(arr[1]),
      userName: dco_decode_String(arr[2]),
      userDisplayName: dco_decode_String(arr[3]),
      challenge: dco_decode_list_prim_u_8_strict(arr[4]),
    );
  }

  @protected
  CsvImportConfigData dco_decode_csv_import_config_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 5)
      throw Exception('unexpected arr length: expect 5 but see ${arr.length}');
    return CsvImportConfigData(
      titleCol: dco_decode_opt_String(arr[0]),
      urlCol: dco_decode_opt_String(arr[1]),
      usernameCol: dco_decode_opt_String(arr[2]),
      passwordCol: dco_decode_opt_String(arr[3]),
      notesCol: dco_decode_opt_String(arr[4]),
    );
  }

  @protected
  CsvPreviewData dco_decode_csv_preview_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2)
      throw Exception('unexpected arr length: expect 2 but see ${arr.length}');
    return CsvPreviewData(
      headers: dco_decode_list_String(arr[0]),
      rows: dco_decode_list_list_String(arr[1]),
    );
  }

  @protected
  CustomEntryData dco_decode_custom_entry_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 7)
      throw Exception('unexpected arr length: expect 7 but see ${arr.length}');
    return CustomEntryData(
      id: dco_decode_String(arr[0]),
      createdAt: dco_decode_String(arr[1]),
      updatedAt: dco_decode_String(arr[2]),
      folder: dco_decode_String(arr[3]),
      title: dco_decode_String(arr[4]),
      fields: dco_decode_list_custom_field_data(arr[5]),
      attachments: dco_decode_list_attachment_meta_data(arr[6]),
    );
  }

  @protected
  CustomFieldData dco_decode_custom_field_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 3)
      throw Exception('unexpected arr length: expect 3 but see ${arr.length}');
    return CustomFieldData(
      label: dco_decode_String(arr[0]),
      value: dco_decode_String(arr[1]),
      hidden: dco_decode_bool(arr[2]),
    );
  }

  @protected
  EntropyResult dco_decode_entropy_result(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2)
      throw Exception('unexpected arr length: expect 2 but see ${arr.length}');
    return EntropyResult(
      bits: dco_decode_f_64(arr[0]),
      tier: dco_decode_strength_tier(arr[1]),
    );
  }

  @protected
  EntrySummaryData dco_decode_entry_summary_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 5)
      throw Exception('unexpected arr length: expect 5 but see ${arr.length}');
    return EntrySummaryData(
      id: dco_decode_String(arr[0]),
      entryType: dco_decode_String(arr[1]),
      title: dco_decode_String(arr[2]),
      folder: dco_decode_String(arr[3]),
      searchBlob: dco_decode_String(arr[4]),
    );
  }

  @protected
  ExportArtifact dco_decode_export_artifact(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2)
      throw Exception('unexpected arr length: expect 2 but see ${arr.length}');
    return ExportArtifact(
      vaultBytes: dco_decode_list_prim_u_8_strict(arr[0]),
      sha256Line: dco_decode_String(arr[1]),
    );
  }

  @protected
  double dco_decode_f_64(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw as double;
  }

  @protected
  FidoCredentialData dco_decode_fido_credential_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2)
      throw Exception('unexpected arr length: expect 2 but see ${arr.length}');
    return FidoCredentialData(
      credentialId: dco_decode_list_prim_u_8_strict(arr[0]),
      salt: dco_decode_list_prim_u_8_strict(arr[1]),
    );
  }

  @protected
  FidoHmacMatch dco_decode_fido_hmac_match(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2)
      throw Exception('unexpected arr length: expect 2 but see ${arr.length}');
    return FidoHmacMatch(
      hmac: dco_decode_list_prim_u_8_strict(arr[0]),
      credentialId: dco_decode_list_prim_u_8_strict(arr[1]),
    );
  }

  @protected
  FidoRecordInput dco_decode_fido_record_input(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2)
      throw Exception('unexpected arr length: expect 2 but see ${arr.length}');
    return FidoRecordInput(
      credentialId: dco_decode_list_prim_u_8_strict(arr[0]),
      salt: dco_decode_list_prim_u_8_strict(arr[1]),
    );
  }

  @protected
  FieldConflictItem dco_decode_field_conflict_item(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 5)
      throw Exception('unexpected arr length: expect 5 but see ${arr.length}');
    return FieldConflictItem(
      id: dco_decode_String(arr[0]),
      title: dco_decode_String(arr[1]),
      field: dco_decode_String(arr[2]),
      localValue: dco_decode_String(arr[3]),
      incomingValue: dco_decode_String(arr[4]),
    );
  }

  @protected
  FileEntryData dco_decode_file_entry_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 8)
      throw Exception('unexpected arr length: expect 8 but see ${arr.length}');
    return FileEntryData(
      id: dco_decode_String(arr[0]),
      createdAt: dco_decode_String(arr[1]),
      updatedAt: dco_decode_String(arr[2]),
      folder: dco_decode_String(arr[3]),
      filename: dco_decode_String(arr[4]),
      data: dco_decode_list_prim_u_8_strict(arr[5]),
      notes: dco_decode_opt_String(arr[6]),
      customFields: dco_decode_list_custom_field_data(arr[7]),
    );
  }

  @protected
  FolderConflictItem dco_decode_folder_conflict_item(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 4)
      throw Exception('unexpected arr length: expect 4 but see ${arr.length}');
    return FolderConflictItem(
      id: dco_decode_String(arr[0]),
      title: dco_decode_String(arr[1]),
      localFolder: dco_decode_String(arr[2]),
      incomingFolder: dco_decode_String(arr[3]),
    );
  }

  @protected
  GabbroImportResult dco_decode_gabbro_import_result(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 1)
      throw Exception('unexpected arr length: expect 1 but see ${arr.length}');
    return GabbroImportResult(imported: dco_decode_usize(arr[0]));
  }

  @protected
  HistoryRecordData dco_decode_history_record_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 4)
      throw Exception('unexpected arr length: expect 4 but see ${arr.length}');
    return HistoryRecordData(
      field: dco_decode_String(arr[0]),
      value: dco_decode_String(arr[1]),
      savedAt: dco_decode_String(arr[2]),
      expiresAt: dco_decode_opt_String(arr[3]),
    );
  }

  @protected
  int dco_decode_i_32(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw as int;
  }

  @protected
  IdentityEntryData dco_decode_identity_entry_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 11)
      throw Exception('unexpected arr length: expect 11 but see ${arr.length}');
    return IdentityEntryData(
      id: dco_decode_String(arr[0]),
      createdAt: dco_decode_String(arr[1]),
      updatedAt: dco_decode_String(arr[2]),
      folder: dco_decode_String(arr[3]),
      firstName: dco_decode_String(arr[4]),
      lastName: dco_decode_String(arr[5]),
      email: dco_decode_String(arr[6]),
      phone: dco_decode_opt_String(arr[7]),
      address: dco_decode_opt_String(arr[8]),
      customFields: dco_decode_list_custom_field_data(arr[9]),
      attachments: dco_decode_list_attachment_meta_data(arr[10]),
    );
  }

  @protected
  ImportFailureData dco_decode_import_failure_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 4)
      throw Exception('unexpected arr length: expect 4 but see ${arr.length}');
    return ImportFailureData(
      title: dco_decode_String(arr[0]),
      category: dco_decode_String(arr[1]),
      reason: dco_decode_String(arr[2]),
      rawFields: dco_decode_list_record_string_string(arr[3]),
    );
  }

  @protected
  ImportResult dco_decode_import_result(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2)
      throw Exception('unexpected arr length: expect 2 but see ${arr.length}');
    return ImportResult(
      imported: dco_decode_usize(arr[0]),
      failures: dco_decode_list_import_failure_data(arr[1]),
    );
  }

  @protected
  Language dco_decode_language(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return Language.values[raw as int];
  }

  @protected
  List<String> dco_decode_list_String(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_String).toList();
  }

  @protected
  List<AddedEntryItem> dco_decode_list_added_entry_item(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_added_entry_item).toList();
  }

  @protected
  List<AttachmentMetaData> dco_decode_list_attachment_meta_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_attachment_meta_data).toList();
  }

  @protected
  List<BroughtOverItem> dco_decode_list_brought_over_item(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_brought_over_item).toList();
  }

  @protected
  List<CustomFieldData> dco_decode_list_custom_field_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_custom_field_data).toList();
  }

  @protected
  List<EntrySummaryData> dco_decode_list_entry_summary_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_entry_summary_data).toList();
  }

  @protected
  List<FidoRecordInput> dco_decode_list_fido_record_input(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_fido_record_input).toList();
  }

  @protected
  List<FieldConflictItem> dco_decode_list_field_conflict_item(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_field_conflict_item).toList();
  }

  @protected
  List<FolderConflictItem> dco_decode_list_folder_conflict_item(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_folder_conflict_item).toList();
  }

  @protected
  List<HistoryRecordData> dco_decode_list_history_record_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_history_record_data).toList();
  }

  @protected
  List<ImportFailureData> dco_decode_list_import_failure_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_import_failure_data).toList();
  }

  @protected
  List<List<String>> dco_decode_list_list_String(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_list_String).toList();
  }

  @protected
  List<Uint8List> dco_decode_list_list_prim_u_8_strict(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_list_prim_u_8_strict).toList();
  }

  @protected
  List<PasskeyMatchData> dco_decode_list_passkey_match_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_passkey_match_data).toList();
  }

  @protected
  List<PendingDeleteItem> dco_decode_list_pending_delete_item(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_pending_delete_item).toList();
  }

  @protected
  List<PendingItemDeleteItem> dco_decode_list_pending_item_delete_item(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>)
        .map(dco_decode_pending_item_delete_item)
        .toList();
  }

  @protected
  List<int> dco_decode_list_prim_u_8_loose(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw as List<int>;
  }

  @protected
  Uint8List dco_decode_list_prim_u_8_strict(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw as Uint8List;
  }

  @protected
  List<(String, String)> dco_decode_list_record_string_string(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_record_string_string).toList();
  }

  @protected
  List<SyncFieldResolutionInput> dco_decode_list_sync_field_resolution_input(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>)
        .map(dco_decode_sync_field_resolution_input)
        .toList();
  }

  @protected
  List<SyncFolderInput> dco_decode_list_sync_folder_input(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_sync_folder_input).toList();
  }

  @protected
  List<SyncHistoryReplacementInput>
  dco_decode_list_sync_history_replacement_input(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>)
        .map(dco_decode_sync_history_replacement_input)
        .toList();
  }

  @protected
  List<SyncItemDeleteInput> dco_decode_list_sync_item_delete_input(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>)
        .map(dco_decode_sync_item_delete_input)
        .toList();
  }

  @protected
  List<YubiKeyInitData> dco_decode_list_yubi_key_init_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_yubi_key_init_data).toList();
  }

  @protected
  List<YubikeyAliasData> dco_decode_list_yubikey_alias_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_yubikey_alias_data).toList();
  }

  @protected
  List<YubikeyRecordData> dco_decode_list_yubikey_record_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_yubikey_record_data).toList();
  }

  @protected
  LoginEntryData dco_decode_login_entry_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 13)
      throw Exception('unexpected arr length: expect 13 but see ${arr.length}');
    return LoginEntryData(
      id: dco_decode_String(arr[0]),
      createdAt: dco_decode_String(arr[1]),
      updatedAt: dco_decode_String(arr[2]),
      folder: dco_decode_String(arr[3]),
      title: dco_decode_String(arr[4]),
      url: dco_decode_String(arr[5]),
      username: dco_decode_String(arr[6]),
      password: dco_decode_String(arr[7]),
      notes: dco_decode_opt_String(arr[8]),
      customFields: dco_decode_list_custom_field_data(arr[9]),
      attachments: dco_decode_list_attachment_meta_data(arr[10]),
      appId: dco_decode_opt_String(arr[11]),
      email: dco_decode_opt_String(arr[12]),
    );
  }

  @protected
  MergeSummary dco_decode_merge_summary(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 8)
      throw Exception('unexpected arr length: expect 8 but see ${arr.length}');
    return MergeSummary(
      added: dco_decode_u_32(arr[0]),
      updated: dco_decode_u_32(arr[1]),
      addedEntries: dco_decode_list_added_entry_item(arr[2]),
      broughtOver: dco_decode_list_brought_over_item(arr[3]),
      pendingDeletes: dco_decode_list_pending_delete_item(arr[4]),
      folderConflicts: dco_decode_list_folder_conflict_item(arr[5]),
      fieldConflicts: dco_decode_list_field_conflict_item(arr[6]),
      pendingItemDeletes: dco_decode_list_pending_item_delete_item(arr[7]),
    );
  }

  @protected
  NoteEntryData dco_decode_note_entry_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 8)
      throw Exception('unexpected arr length: expect 8 but see ${arr.length}');
    return NoteEntryData(
      id: dco_decode_String(arr[0]),
      createdAt: dco_decode_String(arr[1]),
      updatedAt: dco_decode_String(arr[2]),
      folder: dco_decode_String(arr[3]),
      title: dco_decode_String(arr[4]),
      content: dco_decode_String(arr[5]),
      customFields: dco_decode_list_custom_field_data(arr[6]),
      attachments: dco_decode_list_attachment_meta_data(arr[7]),
    );
  }

  @protected
  String? dco_decode_opt_String(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null ? null : dco_decode_String(raw);
  }

  @protected
  CapturedWindowData? dco_decode_opt_box_autoadd_captured_window_data(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null
        ? null
        : dco_decode_box_autoadd_captured_window_data(raw);
  }

  @protected
  MergeSummary? dco_decode_opt_box_autoadd_merge_summary(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null ? null : dco_decode_box_autoadd_merge_summary(raw);
  }

  @protected
  int? dco_decode_opt_box_autoadd_u_32(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null ? null : dco_decode_box_autoadd_u_32(raw);
  }

  @protected
  Uint8List? dco_decode_opt_list_prim_u_8_strict(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null ? null : dco_decode_list_prim_u_8_strict(raw);
  }

  @protected
  PasskeyEntryData dco_decode_passkey_entry_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 10)
      throw Exception('unexpected arr length: expect 10 but see ${arr.length}');
    return PasskeyEntryData(
      id: dco_decode_String(arr[0]),
      createdAt: dco_decode_String(arr[1]),
      updatedAt: dco_decode_String(arr[2]),
      folder: dco_decode_String(arr[3]),
      rpId: dco_decode_String(arr[4]),
      userName: dco_decode_String(arr[5]),
      userDisplayName: dco_decode_String(arr[6]),
      credentialIdB64: dco_decode_String(arr[7]),
      notes: dco_decode_opt_String(arr[8]),
      customFields: dco_decode_list_custom_field_data(arr[9]),
    );
  }

  @protected
  PasskeyMatchData dco_decode_passkey_match_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 5)
      throw Exception('unexpected arr length: expect 5 but see ${arr.length}');
    return PasskeyMatchData(
      entryId: dco_decode_String(arr[0]),
      rpId: dco_decode_String(arr[1]),
      userName: dco_decode_String(arr[2]),
      userDisplayName: dco_decode_String(arr[3]),
      credentialId: dco_decode_list_prim_u_8_strict(arr[4]),
    );
  }

  @protected
  PasskeyPlan dco_decode_passkey_plan(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 4)
      throw Exception('unexpected arr length: expect 4 but see ${arr.length}');
    return PasskeyPlan(
      immediateResponse: dco_decode_opt_list_prim_u_8_strict(arr[0]),
      isCreate: dco_decode_bool(arr[1]),
      rpId: dco_decode_String(arr[2]),
      accounts: dco_decode_list_String(arr[3]),
    );
  }

  @protected
  PassphraseConfig dco_decode_passphrase_config(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 5)
      throw Exception('unexpected arr length: expect 5 but see ${arr.length}');
    return PassphraseConfig(
      wordCount: dco_decode_u_32(arr[0]),
      separator: dco_decode_String(arr[1]),
      capitalise: dco_decode_bool(arr[2]),
      appendNumber: dco_decode_bool(arr[3]),
      language: dco_decode_language(arr[4]),
    );
  }

  @protected
  PasswordConfig dco_decode_password_config(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 7)
      throw Exception('unexpected arr length: expect 7 but see ${arr.length}');
    return PasswordConfig(
      length: dco_decode_u_32(arr[0]),
      useUppercase: dco_decode_bool(arr[1]),
      useLowercase: dco_decode_bool(arr[2]),
      useDigits: dco_decode_bool(arr[3]),
      useSymbols: dco_decode_bool(arr[4]),
      excludeAmbiguous: dco_decode_bool(arr[5]),
      language: dco_decode_language(arr[6]),
    );
  }

  @protected
  PendingDeleteItem dco_decode_pending_delete_item(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2)
      throw Exception('unexpected arr length: expect 2 but see ${arr.length}');
    return PendingDeleteItem(
      id: dco_decode_String(arr[0]),
      title: dco_decode_String(arr[1]),
    );
  }

  @protected
  PendingItemDeleteItem dco_decode_pending_item_delete_item(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 4)
      throw Exception('unexpected arr length: expect 4 but see ${arr.length}');
    return PendingItemDeleteItem(
      id: dco_decode_String(arr[0]),
      title: dco_decode_String(arr[1]),
      field: dco_decode_String(arr[2]),
      label: dco_decode_String(arr[3]),
    );
  }

  @protected
  (String, String) dco_decode_record_string_string(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2) {
      throw Exception('Expected 2 elements, got ${arr.length}');
    }
    return (dco_decode_String(arr[0]), dco_decode_String(arr[1]));
  }

  @protected
  RegistrationParts dco_decode_registration_parts(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 3)
      throw Exception('unexpected arr length: expect 3 but see ${arr.length}');
    return RegistrationParts(
      credentialId: dco_decode_list_prim_u_8_strict(arr[0]),
      authData: dco_decode_list_prim_u_8_strict(arr[1]),
      publicKeyCose: dco_decode_list_prim_u_8_strict(arr[2]),
    );
  }

  @protected
  StrengthTier dco_decode_strength_tier(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return StrengthTier.values[raw as int];
  }

  @protected
  SyncFieldResolutionInput dco_decode_sync_field_resolution_input(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 4)
      throw Exception('unexpected arr length: expect 4 but see ${arr.length}');
    return SyncFieldResolutionInput(
      id: dco_decode_String(arr[0]),
      field: dco_decode_String(arr[1]),
      keepIncoming: dco_decode_bool(arr[2]),
      value: dco_decode_String(arr[3]),
    );
  }

  @protected
  SyncFolderInput dco_decode_sync_folder_input(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2)
      throw Exception('unexpected arr length: expect 2 but see ${arr.length}');
    return SyncFolderInput(
      id: dco_decode_String(arr[0]),
      folder: dco_decode_String(arr[1]),
    );
  }

  @protected
  SyncHistoryReplacementInput dco_decode_sync_history_replacement_input(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 4)
      throw Exception('unexpected arr length: expect 4 but see ${arr.length}');
    return SyncHistoryReplacementInput(
      id: dco_decode_String(arr[0]),
      field: dco_decode_String(arr[1]),
      newValue: dco_decode_String(arr[2]),
      replacedValue: dco_decode_String(arr[3]),
    );
  }

  @protected
  SyncItemDeleteInput dco_decode_sync_item_delete_input(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 3)
      throw Exception('unexpected arr length: expect 3 but see ${arr.length}');
    return SyncItemDeleteInput(
      id: dco_decode_String(arr[0]),
      field: dco_decode_String(arr[1]),
      delete: dco_decode_bool(arr[2]),
    );
  }

  @protected
  int dco_decode_u_32(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw as int;
  }

  @protected
  BigInt dco_decode_u_64(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dcoDecodeU64(raw);
  }

  @protected
  int dco_decode_u_8(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw as int;
  }

  @protected
  void dco_decode_unit(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return;
  }

  @protected
  BigInt dco_decode_usize(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dcoDecodeU64(raw);
  }

  @protected
  VaultEntryData dco_decode_vault_entry_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    switch (raw[0]) {
      case 0:
        return VaultEntryData_Login(
          dco_decode_box_autoadd_login_entry_data(raw[1]),
        );
      case 1:
        return VaultEntryData_Note(
          dco_decode_box_autoadd_note_entry_data(raw[1]),
        );
      case 2:
        return VaultEntryData_Identity(
          dco_decode_box_autoadd_identity_entry_data(raw[1]),
        );
      case 3:
        return VaultEntryData_Card(
          dco_decode_box_autoadd_card_entry_data(raw[1]),
        );
      case 4:
        return VaultEntryData_File(
          dco_decode_box_autoadd_file_entry_data(raw[1]),
        );
      case 5:
        return VaultEntryData_Custom(
          dco_decode_box_autoadd_custom_entry_data(raw[1]),
        );
      case 6:
        return VaultEntryData_Passkey(
          dco_decode_box_autoadd_passkey_entry_data(raw[1]),
        );
      default:
        throw Exception("unreachable");
    }
  }

  @protected
  VaultHeaderData dco_decode_vault_header_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2)
      throw Exception('unexpected arr length: expect 2 but see ${arr.length}');
    return VaultHeaderData(
      alias: dco_decode_opt_String(arr[0]),
      yubikeyRecords: dco_decode_list_yubikey_record_data(arr[1]),
    );
  }

  @protected
  YubiKeyInitData dco_decode_yubi_key_init_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 3)
      throw Exception('unexpected arr length: expect 3 but see ${arr.length}');
    return YubiKeyInitData(
      credentialId: dco_decode_list_prim_u_8_strict(arr[0]),
      hmacSecret: dco_decode_list_prim_u_8_strict(arr[1]),
      hkdfSalt: dco_decode_list_prim_u_8_strict(arr[2]),
    );
  }

  @protected
  YubikeyAliasData dco_decode_yubikey_alias_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2)
      throw Exception('unexpected arr length: expect 2 but see ${arr.length}');
    return YubikeyAliasData(
      credentialIdHex: dco_decode_String(arr[0]),
      alias: dco_decode_String(arr[1]),
    );
  }

  @protected
  YubikeyRecordData dco_decode_yubikey_record_data(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2)
      throw Exception('unexpected arr length: expect 2 but see ${arr.length}');
    return YubikeyRecordData(
      credentialId: dco_decode_list_prim_u_8_strict(arr[0]),
      salt: dco_decode_list_prim_u_8_strict(arr[1]),
    );
  }

  @protected
  AnyhowException sse_decode_AnyhowException(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var inner = sse_decode_String(deserializer);
    return AnyhowException(inner);
  }

  @protected
  RustStreamSink<Uint8List> sse_decode_StreamSink_list_prim_u_8_strict_Sse(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    throw UnimplementedError('Unreachable ()');
  }

  @protected
  String sse_decode_String(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var inner = sse_decode_list_prim_u_8_strict(deserializer);
    return utf8.decoder.convert(inner);
  }

  @protected
  AddedEntryItem sse_decode_added_entry_item(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_id = sse_decode_String(deserializer);
    var var_title = sse_decode_String(deserializer);
    return AddedEntryItem(id: var_id, title: var_title);
  }

  @protected
  AssertionParts sse_decode_assertion_parts(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_credentialId = sse_decode_list_prim_u_8_strict(deserializer);
    var var_authData = sse_decode_list_prim_u_8_strict(deserializer);
    var var_signatureDer = sse_decode_list_prim_u_8_strict(deserializer);
    var var_userHandle = sse_decode_list_prim_u_8_strict(deserializer);
    return AssertionParts(
      credentialId: var_credentialId,
      authData: var_authData,
      signatureDer: var_signatureDer,
      userHandle: var_userHandle,
    );
  }

  @protected
  AssertionRequest sse_decode_assertion_request(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_rpId = sse_decode_String(deserializer);
    var var_challenge = sse_decode_list_prim_u_8_strict(deserializer);
    var var_allowCredentialIds = sse_decode_list_list_prim_u_8_strict(
      deserializer,
    );
    return AssertionRequest(
      rpId: var_rpId,
      challenge: var_challenge,
      allowCredentialIds: var_allowCredentialIds,
    );
  }

  @protected
  AttachmentMetaData sse_decode_attachment_meta_data(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_uuid = sse_decode_String(deserializer);
    var var_name = sse_decode_String(deserializer);
    var var_kind = sse_decode_String(deserializer);
    var var_size = sse_decode_u_64(deserializer);
    return AttachmentMetaData(
      uuid: var_uuid,
      name: var_name,
      kind: var_kind,
      size: var_size,
    );
  }

  @protected
  bool sse_decode_bool(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return deserializer.buffer.getUint8() != 0;
  }

  @protected
  CapturedWindowData sse_decode_box_autoadd_captured_window_data(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_captured_window_data(deserializer));
  }

  @protected
  CardEntryData sse_decode_box_autoadd_card_entry_data(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_card_entry_data(deserializer));
  }

  @protected
  CsvImportConfigData sse_decode_box_autoadd_csv_import_config_data(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_csv_import_config_data(deserializer));
  }

  @protected
  CustomEntryData sse_decode_box_autoadd_custom_entry_data(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_custom_entry_data(deserializer));
  }

  @protected
  FileEntryData sse_decode_box_autoadd_file_entry_data(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_file_entry_data(deserializer));
  }

  @protected
  IdentityEntryData sse_decode_box_autoadd_identity_entry_data(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_identity_entry_data(deserializer));
  }

  @protected
  LoginEntryData sse_decode_box_autoadd_login_entry_data(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_login_entry_data(deserializer));
  }

  @protected
  MergeSummary sse_decode_box_autoadd_merge_summary(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_merge_summary(deserializer));
  }

  @protected
  NoteEntryData sse_decode_box_autoadd_note_entry_data(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_note_entry_data(deserializer));
  }

  @protected
  PasskeyEntryData sse_decode_box_autoadd_passkey_entry_data(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_passkey_entry_data(deserializer));
  }

  @protected
  PassphraseConfig sse_decode_box_autoadd_passphrase_config(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_passphrase_config(deserializer));
  }

  @protected
  PasswordConfig sse_decode_box_autoadd_password_config(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_password_config(deserializer));
  }

  @protected
  int sse_decode_box_autoadd_u_32(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_u_32(deserializer));
  }

  @protected
  VaultEntryData sse_decode_box_autoadd_vault_entry_data(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_vault_entry_data(deserializer));
  }

  @protected
  BroughtOverItem sse_decode_brought_over_item(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_id = sse_decode_String(deserializer);
    var var_title = sse_decode_String(deserializer);
    var var_field = sse_decode_String(deserializer);
    var var_oldValue = sse_decode_String(deserializer);
    var var_newValue = sse_decode_String(deserializer);
    return BroughtOverItem(
      id: var_id,
      title: var_title,
      field: var_field,
      oldValue: var_oldValue,
      newValue: var_newValue,
    );
  }

  @protected
  CapturedWindowData sse_decode_captured_window_data(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_id = sse_decode_u_32(deserializer);
    var var_appClass = sse_decode_String(deserializer);
    var var_title = sse_decode_String(deserializer);
    return CapturedWindowData(
      id: var_id,
      appClass: var_appClass,
      title: var_title,
    );
  }

  @protected
  CardEntryData sse_decode_card_entry_data(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_id = sse_decode_String(deserializer);
    var var_createdAt = sse_decode_String(deserializer);
    var var_updatedAt = sse_decode_String(deserializer);
    var var_folder = sse_decode_String(deserializer);
    var var_cardName = sse_decode_opt_String(deserializer);
    var var_status = sse_decode_String(deserializer);
    var var_cardholderName = sse_decode_String(deserializer);
    var var_cardNumber = sse_decode_String(deserializer);
    var var_expiry = sse_decode_String(deserializer);
    var var_cvv = sse_decode_String(deserializer);
    var var_creditLimit = sse_decode_opt_String(deserializer);
    var var_cardAccountNumber = sse_decode_opt_String(deserializer);
    var var_paymentNetwork = sse_decode_opt_String(deserializer);
    var var_pin = sse_decode_opt_String(deserializer);
    var var_bankName = sse_decode_opt_String(deserializer);
    var var_transactionPassword = sse_decode_opt_String(deserializer);
    var var_notes = sse_decode_opt_String(deserializer);
    var var_customFields = sse_decode_list_custom_field_data(deserializer);
    var var_attachments = sse_decode_list_attachment_meta_data(deserializer);
    return CardEntryData(
      id: var_id,
      createdAt: var_createdAt,
      updatedAt: var_updatedAt,
      folder: var_folder,
      cardName: var_cardName,
      status: var_status,
      cardholderName: var_cardholderName,
      cardNumber: var_cardNumber,
      expiry: var_expiry,
      cvv: var_cvv,
      creditLimit: var_creditLimit,
      cardAccountNumber: var_cardAccountNumber,
      paymentNetwork: var_paymentNetwork,
      pin: var_pin,
      bankName: var_bankName,
      transactionPassword: var_transactionPassword,
      notes: var_notes,
      customFields: var_customFields,
      attachments: var_attachments,
    );
  }

  @protected
  CreationRequest sse_decode_creation_request(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_rpId = sse_decode_String(deserializer);
    var var_userHandle = sse_decode_list_prim_u_8_strict(deserializer);
    var var_userName = sse_decode_String(deserializer);
    var var_userDisplayName = sse_decode_String(deserializer);
    var var_challenge = sse_decode_list_prim_u_8_strict(deserializer);
    return CreationRequest(
      rpId: var_rpId,
      userHandle: var_userHandle,
      userName: var_userName,
      userDisplayName: var_userDisplayName,
      challenge: var_challenge,
    );
  }

  @protected
  CsvImportConfigData sse_decode_csv_import_config_data(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_titleCol = sse_decode_opt_String(deserializer);
    var var_urlCol = sse_decode_opt_String(deserializer);
    var var_usernameCol = sse_decode_opt_String(deserializer);
    var var_passwordCol = sse_decode_opt_String(deserializer);
    var var_notesCol = sse_decode_opt_String(deserializer);
    return CsvImportConfigData(
      titleCol: var_titleCol,
      urlCol: var_urlCol,
      usernameCol: var_usernameCol,
      passwordCol: var_passwordCol,
      notesCol: var_notesCol,
    );
  }

  @protected
  CsvPreviewData sse_decode_csv_preview_data(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_headers = sse_decode_list_String(deserializer);
    var var_rows = sse_decode_list_list_String(deserializer);
    return CsvPreviewData(headers: var_headers, rows: var_rows);
  }

  @protected
  CustomEntryData sse_decode_custom_entry_data(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_id = sse_decode_String(deserializer);
    var var_createdAt = sse_decode_String(deserializer);
    var var_updatedAt = sse_decode_String(deserializer);
    var var_folder = sse_decode_String(deserializer);
    var var_title = sse_decode_String(deserializer);
    var var_fields = sse_decode_list_custom_field_data(deserializer);
    var var_attachments = sse_decode_list_attachment_meta_data(deserializer);
    return CustomEntryData(
      id: var_id,
      createdAt: var_createdAt,
      updatedAt: var_updatedAt,
      folder: var_folder,
      title: var_title,
      fields: var_fields,
      attachments: var_attachments,
    );
  }

  @protected
  CustomFieldData sse_decode_custom_field_data(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_label = sse_decode_String(deserializer);
    var var_value = sse_decode_String(deserializer);
    var var_hidden = sse_decode_bool(deserializer);
    return CustomFieldData(
      label: var_label,
      value: var_value,
      hidden: var_hidden,
    );
  }

  @protected
  EntropyResult sse_decode_entropy_result(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_bits = sse_decode_f_64(deserializer);
    var var_tier = sse_decode_strength_tier(deserializer);
    return EntropyResult(bits: var_bits, tier: var_tier);
  }

  @protected
  EntrySummaryData sse_decode_entry_summary_data(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_id = sse_decode_String(deserializer);
    var var_entryType = sse_decode_String(deserializer);
    var var_title = sse_decode_String(deserializer);
    var var_folder = sse_decode_String(deserializer);
    var var_searchBlob = sse_decode_String(deserializer);
    return EntrySummaryData(
      id: var_id,
      entryType: var_entryType,
      title: var_title,
      folder: var_folder,
      searchBlob: var_searchBlob,
    );
  }

  @protected
  ExportArtifact sse_decode_export_artifact(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_vaultBytes = sse_decode_list_prim_u_8_strict(deserializer);
    var var_sha256Line = sse_decode_String(deserializer);
    return ExportArtifact(
      vaultBytes: var_vaultBytes,
      sha256Line: var_sha256Line,
    );
  }

  @protected
  double sse_decode_f_64(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return deserializer.buffer.getFloat64();
  }

  @protected
  FidoCredentialData sse_decode_fido_credential_data(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_credentialId = sse_decode_list_prim_u_8_strict(deserializer);
    var var_salt = sse_decode_list_prim_u_8_strict(deserializer);
    return FidoCredentialData(credentialId: var_credentialId, salt: var_salt);
  }

  @protected
  FidoHmacMatch sse_decode_fido_hmac_match(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_hmac = sse_decode_list_prim_u_8_strict(deserializer);
    var var_credentialId = sse_decode_list_prim_u_8_strict(deserializer);
    return FidoHmacMatch(hmac: var_hmac, credentialId: var_credentialId);
  }

  @protected
  FidoRecordInput sse_decode_fido_record_input(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_credentialId = sse_decode_list_prim_u_8_strict(deserializer);
    var var_salt = sse_decode_list_prim_u_8_strict(deserializer);
    return FidoRecordInput(credentialId: var_credentialId, salt: var_salt);
  }

  @protected
  FieldConflictItem sse_decode_field_conflict_item(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_id = sse_decode_String(deserializer);
    var var_title = sse_decode_String(deserializer);
    var var_field = sse_decode_String(deserializer);
    var var_localValue = sse_decode_String(deserializer);
    var var_incomingValue = sse_decode_String(deserializer);
    return FieldConflictItem(
      id: var_id,
      title: var_title,
      field: var_field,
      localValue: var_localValue,
      incomingValue: var_incomingValue,
    );
  }

  @protected
  FileEntryData sse_decode_file_entry_data(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_id = sse_decode_String(deserializer);
    var var_createdAt = sse_decode_String(deserializer);
    var var_updatedAt = sse_decode_String(deserializer);
    var var_folder = sse_decode_String(deserializer);
    var var_filename = sse_decode_String(deserializer);
    var var_data = sse_decode_list_prim_u_8_strict(deserializer);
    var var_notes = sse_decode_opt_String(deserializer);
    var var_customFields = sse_decode_list_custom_field_data(deserializer);
    return FileEntryData(
      id: var_id,
      createdAt: var_createdAt,
      updatedAt: var_updatedAt,
      folder: var_folder,
      filename: var_filename,
      data: var_data,
      notes: var_notes,
      customFields: var_customFields,
    );
  }

  @protected
  FolderConflictItem sse_decode_folder_conflict_item(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_id = sse_decode_String(deserializer);
    var var_title = sse_decode_String(deserializer);
    var var_localFolder = sse_decode_String(deserializer);
    var var_incomingFolder = sse_decode_String(deserializer);
    return FolderConflictItem(
      id: var_id,
      title: var_title,
      localFolder: var_localFolder,
      incomingFolder: var_incomingFolder,
    );
  }

  @protected
  GabbroImportResult sse_decode_gabbro_import_result(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_imported = sse_decode_usize(deserializer);
    return GabbroImportResult(imported: var_imported);
  }

  @protected
  HistoryRecordData sse_decode_history_record_data(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_field = sse_decode_String(deserializer);
    var var_value = sse_decode_String(deserializer);
    var var_savedAt = sse_decode_String(deserializer);
    var var_expiresAt = sse_decode_opt_String(deserializer);
    return HistoryRecordData(
      field: var_field,
      value: var_value,
      savedAt: var_savedAt,
      expiresAt: var_expiresAt,
    );
  }

  @protected
  int sse_decode_i_32(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return deserializer.buffer.getInt32();
  }

  @protected
  IdentityEntryData sse_decode_identity_entry_data(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_id = sse_decode_String(deserializer);
    var var_createdAt = sse_decode_String(deserializer);
    var var_updatedAt = sse_decode_String(deserializer);
    var var_folder = sse_decode_String(deserializer);
    var var_firstName = sse_decode_String(deserializer);
    var var_lastName = sse_decode_String(deserializer);
    var var_email = sse_decode_String(deserializer);
    var var_phone = sse_decode_opt_String(deserializer);
    var var_address = sse_decode_opt_String(deserializer);
    var var_customFields = sse_decode_list_custom_field_data(deserializer);
    var var_attachments = sse_decode_list_attachment_meta_data(deserializer);
    return IdentityEntryData(
      id: var_id,
      createdAt: var_createdAt,
      updatedAt: var_updatedAt,
      folder: var_folder,
      firstName: var_firstName,
      lastName: var_lastName,
      email: var_email,
      phone: var_phone,
      address: var_address,
      customFields: var_customFields,
      attachments: var_attachments,
    );
  }

  @protected
  ImportFailureData sse_decode_import_failure_data(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_title = sse_decode_String(deserializer);
    var var_category = sse_decode_String(deserializer);
    var var_reason = sse_decode_String(deserializer);
    var var_rawFields = sse_decode_list_record_string_string(deserializer);
    return ImportFailureData(
      title: var_title,
      category: var_category,
      reason: var_reason,
      rawFields: var_rawFields,
    );
  }

  @protected
  ImportResult sse_decode_import_result(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_imported = sse_decode_usize(deserializer);
    var var_failures = sse_decode_list_import_failure_data(deserializer);
    return ImportResult(imported: var_imported, failures: var_failures);
  }

  @protected
  Language sse_decode_language(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var inner = sse_decode_i_32(deserializer);
    return Language.values[inner];
  }

  @protected
  List<String> sse_decode_list_String(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <String>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_String(deserializer));
    }
    return ans_;
  }

  @protected
  List<AddedEntryItem> sse_decode_list_added_entry_item(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <AddedEntryItem>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_added_entry_item(deserializer));
    }
    return ans_;
  }

  @protected
  List<AttachmentMetaData> sse_decode_list_attachment_meta_data(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <AttachmentMetaData>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_attachment_meta_data(deserializer));
    }
    return ans_;
  }

  @protected
  List<BroughtOverItem> sse_decode_list_brought_over_item(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <BroughtOverItem>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_brought_over_item(deserializer));
    }
    return ans_;
  }

  @protected
  List<CustomFieldData> sse_decode_list_custom_field_data(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <CustomFieldData>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_custom_field_data(deserializer));
    }
    return ans_;
  }

  @protected
  List<EntrySummaryData> sse_decode_list_entry_summary_data(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <EntrySummaryData>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_entry_summary_data(deserializer));
    }
    return ans_;
  }

  @protected
  List<FidoRecordInput> sse_decode_list_fido_record_input(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <FidoRecordInput>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_fido_record_input(deserializer));
    }
    return ans_;
  }

  @protected
  List<FieldConflictItem> sse_decode_list_field_conflict_item(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <FieldConflictItem>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_field_conflict_item(deserializer));
    }
    return ans_;
  }

  @protected
  List<FolderConflictItem> sse_decode_list_folder_conflict_item(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <FolderConflictItem>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_folder_conflict_item(deserializer));
    }
    return ans_;
  }

  @protected
  List<HistoryRecordData> sse_decode_list_history_record_data(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <HistoryRecordData>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_history_record_data(deserializer));
    }
    return ans_;
  }

  @protected
  List<ImportFailureData> sse_decode_list_import_failure_data(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <ImportFailureData>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_import_failure_data(deserializer));
    }
    return ans_;
  }

  @protected
  List<List<String>> sse_decode_list_list_String(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <List<String>>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_list_String(deserializer));
    }
    return ans_;
  }

  @protected
  List<Uint8List> sse_decode_list_list_prim_u_8_strict(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <Uint8List>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_list_prim_u_8_strict(deserializer));
    }
    return ans_;
  }

  @protected
  List<PasskeyMatchData> sse_decode_list_passkey_match_data(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <PasskeyMatchData>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_passkey_match_data(deserializer));
    }
    return ans_;
  }

  @protected
  List<PendingDeleteItem> sse_decode_list_pending_delete_item(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <PendingDeleteItem>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_pending_delete_item(deserializer));
    }
    return ans_;
  }

  @protected
  List<PendingItemDeleteItem> sse_decode_list_pending_item_delete_item(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <PendingItemDeleteItem>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_pending_item_delete_item(deserializer));
    }
    return ans_;
  }

  @protected
  List<int> sse_decode_list_prim_u_8_loose(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var len_ = sse_decode_i_32(deserializer);
    return deserializer.buffer.getUint8List(len_);
  }

  @protected
  Uint8List sse_decode_list_prim_u_8_strict(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var len_ = sse_decode_i_32(deserializer);
    return deserializer.buffer.getUint8List(len_);
  }

  @protected
  List<(String, String)> sse_decode_list_record_string_string(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <(String, String)>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_record_string_string(deserializer));
    }
    return ans_;
  }

  @protected
  List<SyncFieldResolutionInput> sse_decode_list_sync_field_resolution_input(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <SyncFieldResolutionInput>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_sync_field_resolution_input(deserializer));
    }
    return ans_;
  }

  @protected
  List<SyncFolderInput> sse_decode_list_sync_folder_input(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <SyncFolderInput>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_sync_folder_input(deserializer));
    }
    return ans_;
  }

  @protected
  List<SyncHistoryReplacementInput>
  sse_decode_list_sync_history_replacement_input(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <SyncHistoryReplacementInput>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_sync_history_replacement_input(deserializer));
    }
    return ans_;
  }

  @protected
  List<SyncItemDeleteInput> sse_decode_list_sync_item_delete_input(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <SyncItemDeleteInput>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_sync_item_delete_input(deserializer));
    }
    return ans_;
  }

  @protected
  List<YubiKeyInitData> sse_decode_list_yubi_key_init_data(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <YubiKeyInitData>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_yubi_key_init_data(deserializer));
    }
    return ans_;
  }

  @protected
  List<YubikeyAliasData> sse_decode_list_yubikey_alias_data(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <YubikeyAliasData>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_yubikey_alias_data(deserializer));
    }
    return ans_;
  }

  @protected
  List<YubikeyRecordData> sse_decode_list_yubikey_record_data(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <YubikeyRecordData>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_yubikey_record_data(deserializer));
    }
    return ans_;
  }

  @protected
  LoginEntryData sse_decode_login_entry_data(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_id = sse_decode_String(deserializer);
    var var_createdAt = sse_decode_String(deserializer);
    var var_updatedAt = sse_decode_String(deserializer);
    var var_folder = sse_decode_String(deserializer);
    var var_title = sse_decode_String(deserializer);
    var var_url = sse_decode_String(deserializer);
    var var_username = sse_decode_String(deserializer);
    var var_password = sse_decode_String(deserializer);
    var var_notes = sse_decode_opt_String(deserializer);
    var var_customFields = sse_decode_list_custom_field_data(deserializer);
    var var_attachments = sse_decode_list_attachment_meta_data(deserializer);
    var var_appId = sse_decode_opt_String(deserializer);
    var var_email = sse_decode_opt_String(deserializer);
    return LoginEntryData(
      id: var_id,
      createdAt: var_createdAt,
      updatedAt: var_updatedAt,
      folder: var_folder,
      title: var_title,
      url: var_url,
      username: var_username,
      password: var_password,
      notes: var_notes,
      customFields: var_customFields,
      attachments: var_attachments,
      appId: var_appId,
      email: var_email,
    );
  }

  @protected
  MergeSummary sse_decode_merge_summary(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_added = sse_decode_u_32(deserializer);
    var var_updated = sse_decode_u_32(deserializer);
    var var_addedEntries = sse_decode_list_added_entry_item(deserializer);
    var var_broughtOver = sse_decode_list_brought_over_item(deserializer);
    var var_pendingDeletes = sse_decode_list_pending_delete_item(deserializer);
    var var_folderConflicts = sse_decode_list_folder_conflict_item(
      deserializer,
    );
    var var_fieldConflicts = sse_decode_list_field_conflict_item(deserializer);
    var var_pendingItemDeletes = sse_decode_list_pending_item_delete_item(
      deserializer,
    );
    return MergeSummary(
      added: var_added,
      updated: var_updated,
      addedEntries: var_addedEntries,
      broughtOver: var_broughtOver,
      pendingDeletes: var_pendingDeletes,
      folderConflicts: var_folderConflicts,
      fieldConflicts: var_fieldConflicts,
      pendingItemDeletes: var_pendingItemDeletes,
    );
  }

  @protected
  NoteEntryData sse_decode_note_entry_data(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_id = sse_decode_String(deserializer);
    var var_createdAt = sse_decode_String(deserializer);
    var var_updatedAt = sse_decode_String(deserializer);
    var var_folder = sse_decode_String(deserializer);
    var var_title = sse_decode_String(deserializer);
    var var_content = sse_decode_String(deserializer);
    var var_customFields = sse_decode_list_custom_field_data(deserializer);
    var var_attachments = sse_decode_list_attachment_meta_data(deserializer);
    return NoteEntryData(
      id: var_id,
      createdAt: var_createdAt,
      updatedAt: var_updatedAt,
      folder: var_folder,
      title: var_title,
      content: var_content,
      customFields: var_customFields,
      attachments: var_attachments,
    );
  }

  @protected
  String? sse_decode_opt_String(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_String(deserializer));
    } else {
      return null;
    }
  }

  @protected
  CapturedWindowData? sse_decode_opt_box_autoadd_captured_window_data(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_box_autoadd_captured_window_data(deserializer));
    } else {
      return null;
    }
  }

  @protected
  MergeSummary? sse_decode_opt_box_autoadd_merge_summary(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_box_autoadd_merge_summary(deserializer));
    } else {
      return null;
    }
  }

  @protected
  int? sse_decode_opt_box_autoadd_u_32(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_box_autoadd_u_32(deserializer));
    } else {
      return null;
    }
  }

  @protected
  Uint8List? sse_decode_opt_list_prim_u_8_strict(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_list_prim_u_8_strict(deserializer));
    } else {
      return null;
    }
  }

  @protected
  PasskeyEntryData sse_decode_passkey_entry_data(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_id = sse_decode_String(deserializer);
    var var_createdAt = sse_decode_String(deserializer);
    var var_updatedAt = sse_decode_String(deserializer);
    var var_folder = sse_decode_String(deserializer);
    var var_rpId = sse_decode_String(deserializer);
    var var_userName = sse_decode_String(deserializer);
    var var_userDisplayName = sse_decode_String(deserializer);
    var var_credentialIdB64 = sse_decode_String(deserializer);
    var var_notes = sse_decode_opt_String(deserializer);
    var var_customFields = sse_decode_list_custom_field_data(deserializer);
    return PasskeyEntryData(
      id: var_id,
      createdAt: var_createdAt,
      updatedAt: var_updatedAt,
      folder: var_folder,
      rpId: var_rpId,
      userName: var_userName,
      userDisplayName: var_userDisplayName,
      credentialIdB64: var_credentialIdB64,
      notes: var_notes,
      customFields: var_customFields,
    );
  }

  @protected
  PasskeyMatchData sse_decode_passkey_match_data(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_entryId = sse_decode_String(deserializer);
    var var_rpId = sse_decode_String(deserializer);
    var var_userName = sse_decode_String(deserializer);
    var var_userDisplayName = sse_decode_String(deserializer);
    var var_credentialId = sse_decode_list_prim_u_8_strict(deserializer);
    return PasskeyMatchData(
      entryId: var_entryId,
      rpId: var_rpId,
      userName: var_userName,
      userDisplayName: var_userDisplayName,
      credentialId: var_credentialId,
    );
  }

  @protected
  PasskeyPlan sse_decode_passkey_plan(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_immediateResponse = sse_decode_opt_list_prim_u_8_strict(
      deserializer,
    );
    var var_isCreate = sse_decode_bool(deserializer);
    var var_rpId = sse_decode_String(deserializer);
    var var_accounts = sse_decode_list_String(deserializer);
    return PasskeyPlan(
      immediateResponse: var_immediateResponse,
      isCreate: var_isCreate,
      rpId: var_rpId,
      accounts: var_accounts,
    );
  }

  @protected
  PassphraseConfig sse_decode_passphrase_config(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_wordCount = sse_decode_u_32(deserializer);
    var var_separator = sse_decode_String(deserializer);
    var var_capitalise = sse_decode_bool(deserializer);
    var var_appendNumber = sse_decode_bool(deserializer);
    var var_language = sse_decode_language(deserializer);
    return PassphraseConfig(
      wordCount: var_wordCount,
      separator: var_separator,
      capitalise: var_capitalise,
      appendNumber: var_appendNumber,
      language: var_language,
    );
  }

  @protected
  PasswordConfig sse_decode_password_config(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_length = sse_decode_u_32(deserializer);
    var var_useUppercase = sse_decode_bool(deserializer);
    var var_useLowercase = sse_decode_bool(deserializer);
    var var_useDigits = sse_decode_bool(deserializer);
    var var_useSymbols = sse_decode_bool(deserializer);
    var var_excludeAmbiguous = sse_decode_bool(deserializer);
    var var_language = sse_decode_language(deserializer);
    return PasswordConfig(
      length: var_length,
      useUppercase: var_useUppercase,
      useLowercase: var_useLowercase,
      useDigits: var_useDigits,
      useSymbols: var_useSymbols,
      excludeAmbiguous: var_excludeAmbiguous,
      language: var_language,
    );
  }

  @protected
  PendingDeleteItem sse_decode_pending_delete_item(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_id = sse_decode_String(deserializer);
    var var_title = sse_decode_String(deserializer);
    return PendingDeleteItem(id: var_id, title: var_title);
  }

  @protected
  PendingItemDeleteItem sse_decode_pending_item_delete_item(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_id = sse_decode_String(deserializer);
    var var_title = sse_decode_String(deserializer);
    var var_field = sse_decode_String(deserializer);
    var var_label = sse_decode_String(deserializer);
    return PendingItemDeleteItem(
      id: var_id,
      title: var_title,
      field: var_field,
      label: var_label,
    );
  }

  @protected
  (String, String) sse_decode_record_string_string(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_field0 = sse_decode_String(deserializer);
    var var_field1 = sse_decode_String(deserializer);
    return (var_field0, var_field1);
  }

  @protected
  RegistrationParts sse_decode_registration_parts(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_credentialId = sse_decode_list_prim_u_8_strict(deserializer);
    var var_authData = sse_decode_list_prim_u_8_strict(deserializer);
    var var_publicKeyCose = sse_decode_list_prim_u_8_strict(deserializer);
    return RegistrationParts(
      credentialId: var_credentialId,
      authData: var_authData,
      publicKeyCose: var_publicKeyCose,
    );
  }

  @protected
  StrengthTier sse_decode_strength_tier(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var inner = sse_decode_i_32(deserializer);
    return StrengthTier.values[inner];
  }

  @protected
  SyncFieldResolutionInput sse_decode_sync_field_resolution_input(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_id = sse_decode_String(deserializer);
    var var_field = sse_decode_String(deserializer);
    var var_keepIncoming = sse_decode_bool(deserializer);
    var var_value = sse_decode_String(deserializer);
    return SyncFieldResolutionInput(
      id: var_id,
      field: var_field,
      keepIncoming: var_keepIncoming,
      value: var_value,
    );
  }

  @protected
  SyncFolderInput sse_decode_sync_folder_input(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_id = sse_decode_String(deserializer);
    var var_folder = sse_decode_String(deserializer);
    return SyncFolderInput(id: var_id, folder: var_folder);
  }

  @protected
  SyncHistoryReplacementInput sse_decode_sync_history_replacement_input(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_id = sse_decode_String(deserializer);
    var var_field = sse_decode_String(deserializer);
    var var_newValue = sse_decode_String(deserializer);
    var var_replacedValue = sse_decode_String(deserializer);
    return SyncHistoryReplacementInput(
      id: var_id,
      field: var_field,
      newValue: var_newValue,
      replacedValue: var_replacedValue,
    );
  }

  @protected
  SyncItemDeleteInput sse_decode_sync_item_delete_input(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_id = sse_decode_String(deserializer);
    var var_field = sse_decode_String(deserializer);
    var var_delete = sse_decode_bool(deserializer);
    return SyncItemDeleteInput(
      id: var_id,
      field: var_field,
      delete: var_delete,
    );
  }

  @protected
  int sse_decode_u_32(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return deserializer.buffer.getUint32();
  }

  @protected
  BigInt sse_decode_u_64(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return deserializer.buffer.getBigUint64();
  }

  @protected
  int sse_decode_u_8(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return deserializer.buffer.getUint8();
  }

  @protected
  void sse_decode_unit(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
  }

  @protected
  BigInt sse_decode_usize(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return deserializer.buffer.getBigUint64();
  }

  @protected
  VaultEntryData sse_decode_vault_entry_data(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var tag_ = sse_decode_i_32(deserializer);
    switch (tag_) {
      case 0:
        var var_field0 = sse_decode_box_autoadd_login_entry_data(deserializer);
        return VaultEntryData_Login(var_field0);
      case 1:
        var var_field0 = sse_decode_box_autoadd_note_entry_data(deserializer);
        return VaultEntryData_Note(var_field0);
      case 2:
        var var_field0 = sse_decode_box_autoadd_identity_entry_data(
          deserializer,
        );
        return VaultEntryData_Identity(var_field0);
      case 3:
        var var_field0 = sse_decode_box_autoadd_card_entry_data(deserializer);
        return VaultEntryData_Card(var_field0);
      case 4:
        var var_field0 = sse_decode_box_autoadd_file_entry_data(deserializer);
        return VaultEntryData_File(var_field0);
      case 5:
        var var_field0 = sse_decode_box_autoadd_custom_entry_data(deserializer);
        return VaultEntryData_Custom(var_field0);
      case 6:
        var var_field0 = sse_decode_box_autoadd_passkey_entry_data(
          deserializer,
        );
        return VaultEntryData_Passkey(var_field0);
      default:
        throw UnimplementedError('');
    }
  }

  @protected
  VaultHeaderData sse_decode_vault_header_data(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_alias = sse_decode_opt_String(deserializer);
    var var_yubikeyRecords = sse_decode_list_yubikey_record_data(deserializer);
    return VaultHeaderData(
      alias: var_alias,
      yubikeyRecords: var_yubikeyRecords,
    );
  }

  @protected
  YubiKeyInitData sse_decode_yubi_key_init_data(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_credentialId = sse_decode_list_prim_u_8_strict(deserializer);
    var var_hmacSecret = sse_decode_list_prim_u_8_strict(deserializer);
    var var_hkdfSalt = sse_decode_list_prim_u_8_strict(deserializer);
    return YubiKeyInitData(
      credentialId: var_credentialId,
      hmacSecret: var_hmacSecret,
      hkdfSalt: var_hkdfSalt,
    );
  }

  @protected
  YubikeyAliasData sse_decode_yubikey_alias_data(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_credentialIdHex = sse_decode_String(deserializer);
    var var_alias = sse_decode_String(deserializer);
    return YubikeyAliasData(
      credentialIdHex: var_credentialIdHex,
      alias: var_alias,
    );
  }

  @protected
  YubikeyRecordData sse_decode_yubikey_record_data(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_credentialId = sse_decode_list_prim_u_8_strict(deserializer);
    var var_salt = sse_decode_list_prim_u_8_strict(deserializer);
    return YubikeyRecordData(credentialId: var_credentialId, salt: var_salt);
  }

  @protected
  void sse_encode_AnyhowException(
    AnyhowException self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.message, serializer);
  }

  @protected
  void sse_encode_StreamSink_list_prim_u_8_strict_Sse(
    RustStreamSink<Uint8List> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(
      self.setupAndSerialize(
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_prim_u_8_strict,
          decodeErrorData: sse_decode_AnyhowException,
        ),
      ),
      serializer,
    );
  }

  @protected
  void sse_encode_String(String self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_list_prim_u_8_strict(utf8.encoder.convert(self), serializer);
  }

  @protected
  void sse_encode_added_entry_item(
    AddedEntryItem self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.id, serializer);
    sse_encode_String(self.title, serializer);
  }

  @protected
  void sse_encode_assertion_parts(
    AssertionParts self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_list_prim_u_8_strict(self.credentialId, serializer);
    sse_encode_list_prim_u_8_strict(self.authData, serializer);
    sse_encode_list_prim_u_8_strict(self.signatureDer, serializer);
    sse_encode_list_prim_u_8_strict(self.userHandle, serializer);
  }

  @protected
  void sse_encode_assertion_request(
    AssertionRequest self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.rpId, serializer);
    sse_encode_list_prim_u_8_strict(self.challenge, serializer);
    sse_encode_list_list_prim_u_8_strict(self.allowCredentialIds, serializer);
  }

  @protected
  void sse_encode_attachment_meta_data(
    AttachmentMetaData self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.uuid, serializer);
    sse_encode_String(self.name, serializer);
    sse_encode_String(self.kind, serializer);
    sse_encode_u_64(self.size, serializer);
  }

  @protected
  void sse_encode_bool(bool self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    serializer.buffer.putUint8(self ? 1 : 0);
  }

  @protected
  void sse_encode_box_autoadd_captured_window_data(
    CapturedWindowData self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_captured_window_data(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_card_entry_data(
    CardEntryData self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_card_entry_data(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_csv_import_config_data(
    CsvImportConfigData self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_csv_import_config_data(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_custom_entry_data(
    CustomEntryData self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_custom_entry_data(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_file_entry_data(
    FileEntryData self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_file_entry_data(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_identity_entry_data(
    IdentityEntryData self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_identity_entry_data(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_login_entry_data(
    LoginEntryData self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_login_entry_data(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_merge_summary(
    MergeSummary self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_merge_summary(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_note_entry_data(
    NoteEntryData self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_note_entry_data(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_passkey_entry_data(
    PasskeyEntryData self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_passkey_entry_data(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_passphrase_config(
    PassphraseConfig self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_passphrase_config(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_password_config(
    PasswordConfig self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_password_config(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_u_32(int self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_u_32(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_vault_entry_data(
    VaultEntryData self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_vault_entry_data(self, serializer);
  }

  @protected
  void sse_encode_brought_over_item(
    BroughtOverItem self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.id, serializer);
    sse_encode_String(self.title, serializer);
    sse_encode_String(self.field, serializer);
    sse_encode_String(self.oldValue, serializer);
    sse_encode_String(self.newValue, serializer);
  }

  @protected
  void sse_encode_captured_window_data(
    CapturedWindowData self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_u_32(self.id, serializer);
    sse_encode_String(self.appClass, serializer);
    sse_encode_String(self.title, serializer);
  }

  @protected
  void sse_encode_card_entry_data(
    CardEntryData self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.id, serializer);
    sse_encode_String(self.createdAt, serializer);
    sse_encode_String(self.updatedAt, serializer);
    sse_encode_String(self.folder, serializer);
    sse_encode_opt_String(self.cardName, serializer);
    sse_encode_String(self.status, serializer);
    sse_encode_String(self.cardholderName, serializer);
    sse_encode_String(self.cardNumber, serializer);
    sse_encode_String(self.expiry, serializer);
    sse_encode_String(self.cvv, serializer);
    sse_encode_opt_String(self.creditLimit, serializer);
    sse_encode_opt_String(self.cardAccountNumber, serializer);
    sse_encode_opt_String(self.paymentNetwork, serializer);
    sse_encode_opt_String(self.pin, serializer);
    sse_encode_opt_String(self.bankName, serializer);
    sse_encode_opt_String(self.transactionPassword, serializer);
    sse_encode_opt_String(self.notes, serializer);
    sse_encode_list_custom_field_data(self.customFields, serializer);
    sse_encode_list_attachment_meta_data(self.attachments, serializer);
  }

  @protected
  void sse_encode_creation_request(
    CreationRequest self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.rpId, serializer);
    sse_encode_list_prim_u_8_strict(self.userHandle, serializer);
    sse_encode_String(self.userName, serializer);
    sse_encode_String(self.userDisplayName, serializer);
    sse_encode_list_prim_u_8_strict(self.challenge, serializer);
  }

  @protected
  void sse_encode_csv_import_config_data(
    CsvImportConfigData self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_opt_String(self.titleCol, serializer);
    sse_encode_opt_String(self.urlCol, serializer);
    sse_encode_opt_String(self.usernameCol, serializer);
    sse_encode_opt_String(self.passwordCol, serializer);
    sse_encode_opt_String(self.notesCol, serializer);
  }

  @protected
  void sse_encode_csv_preview_data(
    CsvPreviewData self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_list_String(self.headers, serializer);
    sse_encode_list_list_String(self.rows, serializer);
  }

  @protected
  void sse_encode_custom_entry_data(
    CustomEntryData self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.id, serializer);
    sse_encode_String(self.createdAt, serializer);
    sse_encode_String(self.updatedAt, serializer);
    sse_encode_String(self.folder, serializer);
    sse_encode_String(self.title, serializer);
    sse_encode_list_custom_field_data(self.fields, serializer);
    sse_encode_list_attachment_meta_data(self.attachments, serializer);
  }

  @protected
  void sse_encode_custom_field_data(
    CustomFieldData self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.label, serializer);
    sse_encode_String(self.value, serializer);
    sse_encode_bool(self.hidden, serializer);
  }

  @protected
  void sse_encode_entropy_result(EntropyResult self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_f_64(self.bits, serializer);
    sse_encode_strength_tier(self.tier, serializer);
  }

  @protected
  void sse_encode_entry_summary_data(
    EntrySummaryData self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.id, serializer);
    sse_encode_String(self.entryType, serializer);
    sse_encode_String(self.title, serializer);
    sse_encode_String(self.folder, serializer);
    sse_encode_String(self.searchBlob, serializer);
  }

  @protected
  void sse_encode_export_artifact(
    ExportArtifact self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_list_prim_u_8_strict(self.vaultBytes, serializer);
    sse_encode_String(self.sha256Line, serializer);
  }

  @protected
  void sse_encode_f_64(double self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    serializer.buffer.putFloat64(self);
  }

  @protected
  void sse_encode_fido_credential_data(
    FidoCredentialData self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_list_prim_u_8_strict(self.credentialId, serializer);
    sse_encode_list_prim_u_8_strict(self.salt, serializer);
  }

  @protected
  void sse_encode_fido_hmac_match(
    FidoHmacMatch self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_list_prim_u_8_strict(self.hmac, serializer);
    sse_encode_list_prim_u_8_strict(self.credentialId, serializer);
  }

  @protected
  void sse_encode_fido_record_input(
    FidoRecordInput self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_list_prim_u_8_strict(self.credentialId, serializer);
    sse_encode_list_prim_u_8_strict(self.salt, serializer);
  }

  @protected
  void sse_encode_field_conflict_item(
    FieldConflictItem self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.id, serializer);
    sse_encode_String(self.title, serializer);
    sse_encode_String(self.field, serializer);
    sse_encode_String(self.localValue, serializer);
    sse_encode_String(self.incomingValue, serializer);
  }

  @protected
  void sse_encode_file_entry_data(
    FileEntryData self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.id, serializer);
    sse_encode_String(self.createdAt, serializer);
    sse_encode_String(self.updatedAt, serializer);
    sse_encode_String(self.folder, serializer);
    sse_encode_String(self.filename, serializer);
    sse_encode_list_prim_u_8_strict(self.data, serializer);
    sse_encode_opt_String(self.notes, serializer);
    sse_encode_list_custom_field_data(self.customFields, serializer);
  }

  @protected
  void sse_encode_folder_conflict_item(
    FolderConflictItem self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.id, serializer);
    sse_encode_String(self.title, serializer);
    sse_encode_String(self.localFolder, serializer);
    sse_encode_String(self.incomingFolder, serializer);
  }

  @protected
  void sse_encode_gabbro_import_result(
    GabbroImportResult self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_usize(self.imported, serializer);
  }

  @protected
  void sse_encode_history_record_data(
    HistoryRecordData self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.field, serializer);
    sse_encode_String(self.value, serializer);
    sse_encode_String(self.savedAt, serializer);
    sse_encode_opt_String(self.expiresAt, serializer);
  }

  @protected
  void sse_encode_i_32(int self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    serializer.buffer.putInt32(self);
  }

  @protected
  void sse_encode_identity_entry_data(
    IdentityEntryData self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.id, serializer);
    sse_encode_String(self.createdAt, serializer);
    sse_encode_String(self.updatedAt, serializer);
    sse_encode_String(self.folder, serializer);
    sse_encode_String(self.firstName, serializer);
    sse_encode_String(self.lastName, serializer);
    sse_encode_String(self.email, serializer);
    sse_encode_opt_String(self.phone, serializer);
    sse_encode_opt_String(self.address, serializer);
    sse_encode_list_custom_field_data(self.customFields, serializer);
    sse_encode_list_attachment_meta_data(self.attachments, serializer);
  }

  @protected
  void sse_encode_import_failure_data(
    ImportFailureData self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.title, serializer);
    sse_encode_String(self.category, serializer);
    sse_encode_String(self.reason, serializer);
    sse_encode_list_record_string_string(self.rawFields, serializer);
  }

  @protected
  void sse_encode_import_result(ImportResult self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_usize(self.imported, serializer);
    sse_encode_list_import_failure_data(self.failures, serializer);
  }

  @protected
  void sse_encode_language(Language self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.index, serializer);
  }

  @protected
  void sse_encode_list_String(List<String> self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_String(item, serializer);
    }
  }

  @protected
  void sse_encode_list_added_entry_item(
    List<AddedEntryItem> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_added_entry_item(item, serializer);
    }
  }

  @protected
  void sse_encode_list_attachment_meta_data(
    List<AttachmentMetaData> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_attachment_meta_data(item, serializer);
    }
  }

  @protected
  void sse_encode_list_brought_over_item(
    List<BroughtOverItem> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_brought_over_item(item, serializer);
    }
  }

  @protected
  void sse_encode_list_custom_field_data(
    List<CustomFieldData> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_custom_field_data(item, serializer);
    }
  }

  @protected
  void sse_encode_list_entry_summary_data(
    List<EntrySummaryData> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_entry_summary_data(item, serializer);
    }
  }

  @protected
  void sse_encode_list_fido_record_input(
    List<FidoRecordInput> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_fido_record_input(item, serializer);
    }
  }

  @protected
  void sse_encode_list_field_conflict_item(
    List<FieldConflictItem> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_field_conflict_item(item, serializer);
    }
  }

  @protected
  void sse_encode_list_folder_conflict_item(
    List<FolderConflictItem> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_folder_conflict_item(item, serializer);
    }
  }

  @protected
  void sse_encode_list_history_record_data(
    List<HistoryRecordData> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_history_record_data(item, serializer);
    }
  }

  @protected
  void sse_encode_list_import_failure_data(
    List<ImportFailureData> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_import_failure_data(item, serializer);
    }
  }

  @protected
  void sse_encode_list_list_String(
    List<List<String>> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_list_String(item, serializer);
    }
  }

  @protected
  void sse_encode_list_list_prim_u_8_strict(
    List<Uint8List> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_list_prim_u_8_strict(item, serializer);
    }
  }

  @protected
  void sse_encode_list_passkey_match_data(
    List<PasskeyMatchData> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_passkey_match_data(item, serializer);
    }
  }

  @protected
  void sse_encode_list_pending_delete_item(
    List<PendingDeleteItem> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_pending_delete_item(item, serializer);
    }
  }

  @protected
  void sse_encode_list_pending_item_delete_item(
    List<PendingItemDeleteItem> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_pending_item_delete_item(item, serializer);
    }
  }

  @protected
  void sse_encode_list_prim_u_8_loose(
    List<int> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    serializer.buffer.putUint8List(
      self is Uint8List ? self : Uint8List.fromList(self),
    );
  }

  @protected
  void sse_encode_list_prim_u_8_strict(
    Uint8List self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    serializer.buffer.putUint8List(self);
  }

  @protected
  void sse_encode_list_record_string_string(
    List<(String, String)> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_record_string_string(item, serializer);
    }
  }

  @protected
  void sse_encode_list_sync_field_resolution_input(
    List<SyncFieldResolutionInput> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_sync_field_resolution_input(item, serializer);
    }
  }

  @protected
  void sse_encode_list_sync_folder_input(
    List<SyncFolderInput> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_sync_folder_input(item, serializer);
    }
  }

  @protected
  void sse_encode_list_sync_history_replacement_input(
    List<SyncHistoryReplacementInput> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_sync_history_replacement_input(item, serializer);
    }
  }

  @protected
  void sse_encode_list_sync_item_delete_input(
    List<SyncItemDeleteInput> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_sync_item_delete_input(item, serializer);
    }
  }

  @protected
  void sse_encode_list_yubi_key_init_data(
    List<YubiKeyInitData> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_yubi_key_init_data(item, serializer);
    }
  }

  @protected
  void sse_encode_list_yubikey_alias_data(
    List<YubikeyAliasData> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_yubikey_alias_data(item, serializer);
    }
  }

  @protected
  void sse_encode_list_yubikey_record_data(
    List<YubikeyRecordData> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_yubikey_record_data(item, serializer);
    }
  }

  @protected
  void sse_encode_login_entry_data(
    LoginEntryData self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.id, serializer);
    sse_encode_String(self.createdAt, serializer);
    sse_encode_String(self.updatedAt, serializer);
    sse_encode_String(self.folder, serializer);
    sse_encode_String(self.title, serializer);
    sse_encode_String(self.url, serializer);
    sse_encode_String(self.username, serializer);
    sse_encode_String(self.password, serializer);
    sse_encode_opt_String(self.notes, serializer);
    sse_encode_list_custom_field_data(self.customFields, serializer);
    sse_encode_list_attachment_meta_data(self.attachments, serializer);
    sse_encode_opt_String(self.appId, serializer);
    sse_encode_opt_String(self.email, serializer);
  }

  @protected
  void sse_encode_merge_summary(MergeSummary self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_u_32(self.added, serializer);
    sse_encode_u_32(self.updated, serializer);
    sse_encode_list_added_entry_item(self.addedEntries, serializer);
    sse_encode_list_brought_over_item(self.broughtOver, serializer);
    sse_encode_list_pending_delete_item(self.pendingDeletes, serializer);
    sse_encode_list_folder_conflict_item(self.folderConflicts, serializer);
    sse_encode_list_field_conflict_item(self.fieldConflicts, serializer);
    sse_encode_list_pending_item_delete_item(
      self.pendingItemDeletes,
      serializer,
    );
  }

  @protected
  void sse_encode_note_entry_data(
    NoteEntryData self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.id, serializer);
    sse_encode_String(self.createdAt, serializer);
    sse_encode_String(self.updatedAt, serializer);
    sse_encode_String(self.folder, serializer);
    sse_encode_String(self.title, serializer);
    sse_encode_String(self.content, serializer);
    sse_encode_list_custom_field_data(self.customFields, serializer);
    sse_encode_list_attachment_meta_data(self.attachments, serializer);
  }

  @protected
  void sse_encode_opt_String(String? self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_String(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_box_autoadd_captured_window_data(
    CapturedWindowData? self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_box_autoadd_captured_window_data(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_box_autoadd_merge_summary(
    MergeSummary? self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_box_autoadd_merge_summary(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_box_autoadd_u_32(int? self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_box_autoadd_u_32(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_list_prim_u_8_strict(
    Uint8List? self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_list_prim_u_8_strict(self, serializer);
    }
  }

  @protected
  void sse_encode_passkey_entry_data(
    PasskeyEntryData self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.id, serializer);
    sse_encode_String(self.createdAt, serializer);
    sse_encode_String(self.updatedAt, serializer);
    sse_encode_String(self.folder, serializer);
    sse_encode_String(self.rpId, serializer);
    sse_encode_String(self.userName, serializer);
    sse_encode_String(self.userDisplayName, serializer);
    sse_encode_String(self.credentialIdB64, serializer);
    sse_encode_opt_String(self.notes, serializer);
    sse_encode_list_custom_field_data(self.customFields, serializer);
  }

  @protected
  void sse_encode_passkey_match_data(
    PasskeyMatchData self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.entryId, serializer);
    sse_encode_String(self.rpId, serializer);
    sse_encode_String(self.userName, serializer);
    sse_encode_String(self.userDisplayName, serializer);
    sse_encode_list_prim_u_8_strict(self.credentialId, serializer);
  }

  @protected
  void sse_encode_passkey_plan(PasskeyPlan self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_opt_list_prim_u_8_strict(self.immediateResponse, serializer);
    sse_encode_bool(self.isCreate, serializer);
    sse_encode_String(self.rpId, serializer);
    sse_encode_list_String(self.accounts, serializer);
  }

  @protected
  void sse_encode_passphrase_config(
    PassphraseConfig self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_u_32(self.wordCount, serializer);
    sse_encode_String(self.separator, serializer);
    sse_encode_bool(self.capitalise, serializer);
    sse_encode_bool(self.appendNumber, serializer);
    sse_encode_language(self.language, serializer);
  }

  @protected
  void sse_encode_password_config(
    PasswordConfig self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_u_32(self.length, serializer);
    sse_encode_bool(self.useUppercase, serializer);
    sse_encode_bool(self.useLowercase, serializer);
    sse_encode_bool(self.useDigits, serializer);
    sse_encode_bool(self.useSymbols, serializer);
    sse_encode_bool(self.excludeAmbiguous, serializer);
    sse_encode_language(self.language, serializer);
  }

  @protected
  void sse_encode_pending_delete_item(
    PendingDeleteItem self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.id, serializer);
    sse_encode_String(self.title, serializer);
  }

  @protected
  void sse_encode_pending_item_delete_item(
    PendingItemDeleteItem self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.id, serializer);
    sse_encode_String(self.title, serializer);
    sse_encode_String(self.field, serializer);
    sse_encode_String(self.label, serializer);
  }

  @protected
  void sse_encode_record_string_string(
    (String, String) self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.$1, serializer);
    sse_encode_String(self.$2, serializer);
  }

  @protected
  void sse_encode_registration_parts(
    RegistrationParts self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_list_prim_u_8_strict(self.credentialId, serializer);
    sse_encode_list_prim_u_8_strict(self.authData, serializer);
    sse_encode_list_prim_u_8_strict(self.publicKeyCose, serializer);
  }

  @protected
  void sse_encode_strength_tier(StrengthTier self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.index, serializer);
  }

  @protected
  void sse_encode_sync_field_resolution_input(
    SyncFieldResolutionInput self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.id, serializer);
    sse_encode_String(self.field, serializer);
    sse_encode_bool(self.keepIncoming, serializer);
    sse_encode_String(self.value, serializer);
  }

  @protected
  void sse_encode_sync_folder_input(
    SyncFolderInput self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.id, serializer);
    sse_encode_String(self.folder, serializer);
  }

  @protected
  void sse_encode_sync_history_replacement_input(
    SyncHistoryReplacementInput self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.id, serializer);
    sse_encode_String(self.field, serializer);
    sse_encode_String(self.newValue, serializer);
    sse_encode_String(self.replacedValue, serializer);
  }

  @protected
  void sse_encode_sync_item_delete_input(
    SyncItemDeleteInput self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.id, serializer);
    sse_encode_String(self.field, serializer);
    sse_encode_bool(self.delete, serializer);
  }

  @protected
  void sse_encode_u_32(int self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    serializer.buffer.putUint32(self);
  }

  @protected
  void sse_encode_u_64(BigInt self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    serializer.buffer.putBigUint64(self);
  }

  @protected
  void sse_encode_u_8(int self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    serializer.buffer.putUint8(self);
  }

  @protected
  void sse_encode_unit(void self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
  }

  @protected
  void sse_encode_usize(BigInt self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    serializer.buffer.putBigUint64(self);
  }

  @protected
  void sse_encode_vault_entry_data(
    VaultEntryData self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    switch (self) {
      case VaultEntryData_Login(field0: final field0):
        sse_encode_i_32(0, serializer);
        sse_encode_box_autoadd_login_entry_data(field0, serializer);
      case VaultEntryData_Note(field0: final field0):
        sse_encode_i_32(1, serializer);
        sse_encode_box_autoadd_note_entry_data(field0, serializer);
      case VaultEntryData_Identity(field0: final field0):
        sse_encode_i_32(2, serializer);
        sse_encode_box_autoadd_identity_entry_data(field0, serializer);
      case VaultEntryData_Card(field0: final field0):
        sse_encode_i_32(3, serializer);
        sse_encode_box_autoadd_card_entry_data(field0, serializer);
      case VaultEntryData_File(field0: final field0):
        sse_encode_i_32(4, serializer);
        sse_encode_box_autoadd_file_entry_data(field0, serializer);
      case VaultEntryData_Custom(field0: final field0):
        sse_encode_i_32(5, serializer);
        sse_encode_box_autoadd_custom_entry_data(field0, serializer);
      case VaultEntryData_Passkey(field0: final field0):
        sse_encode_i_32(6, serializer);
        sse_encode_box_autoadd_passkey_entry_data(field0, serializer);
    }
  }

  @protected
  void sse_encode_vault_header_data(
    VaultHeaderData self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_opt_String(self.alias, serializer);
    sse_encode_list_yubikey_record_data(self.yubikeyRecords, serializer);
  }

  @protected
  void sse_encode_yubi_key_init_data(
    YubiKeyInitData self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_list_prim_u_8_strict(self.credentialId, serializer);
    sse_encode_list_prim_u_8_strict(self.hmacSecret, serializer);
    sse_encode_list_prim_u_8_strict(self.hkdfSalt, serializer);
  }

  @protected
  void sse_encode_yubikey_alias_data(
    YubikeyAliasData self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.credentialIdHex, serializer);
    sse_encode_String(self.alias, serializer);
  }

  @protected
  void sse_encode_yubikey_record_data(
    YubikeyRecordData self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_list_prim_u_8_strict(self.credentialId, serializer);
    sse_encode_list_prim_u_8_strict(self.salt, serializer);
  }
}

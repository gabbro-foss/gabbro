...
class YubiKeyManagerTest {
    ...
    fun cross_usb_register_nfc_getHmac_returns_same_bytes() {
        // When: register() called via USB-C to get a credentialId, then getHmacSecret()
        //       called via USB-C and via NFC with the same credentialId and salt
    ...
    fun cross_nfc_register_usb_getHmac_returns_same_bytes() {
        // When: register() called via NFC to get a credentialId, then getHmacSecret()
        //       called via NFC and via USB-C with the same credentialId and salt
        // Then: both hmac-secret outputs are byte-for-byte equal
    }
}
